...
**Current version: v2.7.2 — ~2975 ± 25 CCRL** (5D transposition-table redesign: 4-entry cache-line clustering, generation aging, cached static eval +24% NPS, sticky ttPv flag — **+37.9 ± 15.0 pooled SPRT** over two independent H1 runs, +48 ± 23 relative at LTC; the largest search gain since v2.3.0). See [CHANGELOG](CHANGELOG.md).
...
- **Next** — 5G-rest capture history in main ordering + futility (v2.7.5) · 5H aspiration/draw-detect/check-extension · Syzygy tablebases (before NNUE — reference order: in-play endgame strength + datagen adjudication/WDL relabeling) · NNUE training · Lazy SMP · competition book last · see [ROADMAP.md](ROADMAP.md).
- **v2.7.4** — 5F ProbCut rework: entry from depth 3 at any node type (was non-PV, depth 5), improving-aware margin `beta + 150 − 40×improving` and improving-aware verification depth (`depth−5` / `depth−3`), reference SEE filter, fail-soft return with the margin peeled off plus a TT store of the verified fail-high, and a small ProbCut on shallow TT lower bounds gated to `!inCheck` (ungated it cost 16 WAC points — our captures-only quiescence makes in-check lower bounds untrustworthy). New **capture-history** table with gravity updates, feeding ProbCut's capture ordering. `cutNode` plumbed through the search, consumer-less beyond ProbCut, ready for the 5C/IIR revisits. The reference IIR shape measured +22% nodes in isolation and was NOT shipped. −5.8% nodes, −9.7%/−4.9% wall time to depth, NPS neutral, **WAC 269/300 (new record)**. 🔲 (SPRT vs v2.7.2 pending)
...
**Versión actual: v2.7.2 — ~2975 ± 25 CCRL** (bloque 5D: rediseño de la transposition table — clustering de 4 entradas por línea de caché, aging por generación, eval estático cacheado +24% nps, flag ttPv pegajoso — **+37.9 ± 15.0 SPRT agrupado** en dos runs H1 independientes, +48 ± 23 relativo a LTC; la mayor ganancia de búsqueda desde la v2.3.0). Ver [CHANGELOG](CHANGELOG.md).
...
- **Siguiente** — resto de 5G: capture history en la ordenación principal y en futility (v2.7.5) · 5H aspiration/draw-detect/check-extension · Tablebases Syzygy (antes del NNUE — orden de la referencia: fuerza en finales ya + adjudicación/re-etiquetado WDL del datagen) · Entrenamiento NNUE · Lazy SMP · libro de competición al final · ver [ROADMAP.md](ROADMAP.md).
- **v2.7.4** — Rework de ProbCut (5F): entrada desde depth 3 en cualquier tipo de nodo (antes solo non-PV desde depth 5), margen sensible al flag improving (`beta + 150 − 40×improving`) y profundidad de verificación también sensible (`depth−5` / `depth−3`), filtro SEE de la referencia, retorno fail-soft con el margen descontado más store del fail-high verificado en la TT, y un small ProbCut sobre lower bounds poco profundos de la TT **restringido a `!inCheck`** (sin restringir costaba 16 puntos de WAC: nuestra quiescence es solo-capturas y sus lower bounds en jaque no son fiables). Nueva tabla de **capture history** con updates por gravedad, que alimenta la ordenación de capturas de ProbCut. `cutNode` propagado por toda la búsqueda, sin más consumidor que ProbCut, listo para las revisitas de 5C/IIR. La forma de IIR de la referencia midió +22% de nodos en aislamiento y NO se embarca. −5.8% nodos, −9.7%/−4.9% de tiempo real hasta profundidad, NPS neutro, **WAC 269/300 (récord)**. 🔲 (SPRT vs v2.7.2 pendiente)
