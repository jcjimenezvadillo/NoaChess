...
**Current version: v2.8.2 — SPRT H1, +28.0 ±17.2 Elo vs v2.8.1; ~3013 ±30 CCRL (624-game calibrated LTC gauntlet)** (pawn correction history, verified ProbCut, fixed aspiration with fail-low recentering, continuation-history gravity with proven killer/counter bands, and explicit-only lifecycle-safe UCI logging). See [CHANGELOG](CHANGELOG.md).
...
- **Search block 5 is CLOSED.** Shipped: 5A improving, 5B scope-cut NMP/RFP, 5D TT redesign, corrected quiescence, capture-history ordering, verified ProbCut and continuation-history gravity. Measured and cut: 5C reductions, 5E singular, multi-level continuation blending, multi-cut, dynamic NMP R, adaptive initial aspiration, unconditional check extension and continuous killer/counter bonuses. The final v2.8.2 package passed H1; speculative components were retained only when the combined test supported them.
...
**Versión actual: v2.8.2 — SPRT H1, +28.0 ±17.2 Elo frente a v2.8.1; ~3013 ±30 CCRL (gauntlet LTC calibrado de 624 partidas)** (pawn correction history, ProbCut verificado, aspiración fija con recentrado tras fail-low, gravedad en continuation history con las bandas killer/counter probadas y log UCI exclusivamente explícito y seguro). Ver [CHANGELOG](CHANGELOG.md).
