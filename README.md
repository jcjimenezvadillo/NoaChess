...
**Current version: v2.6.7.1 — ~2890 ± 25 CCRL** (time-management patch: opening damp + neutral first-move factors + UCI ponder/infinite protocol fix; +14.3 ± 13.5 Elo SPRT vs v2.6.7, LOS 98.1%; LTC gauntlet 60+0.6, 448 games, 6 clean anchors). See [CHANGELOG](CHANGELOG.md).
...
- **v2.6.7.1** — Time-management patch (opening damp, neutral first-move factors — 3+2 first move 19s → 6s) + UCI ponder/infinite protocol fix (no bestmove leak on self-terminated searches — the Arena "frozen after new game" bug) + fault-proof command loop. ✅ (+14.3 ± 13.5 Elo SPRT vs v2.6.7, LOS 98.1%; **~2890 ± 25 CCRL**)
...
**Versión actual: v2.6.7.1 — ~2890 ± 25 CCRL** (parche de gestión de tiempo: freno de apertura + factores neutros de primer movimiento + fix del protocolo UCI ponder/infinite; +14.3 ± 13.5 Elo SPRT vs v2.6.7, LOS 98.1%; gauntlet LTC 60+0.6, 448 partidas, 6 anclas limpias). Ver [CHANGELOG](CHANGELOG.md).
...
- **v2.6.7.1** — Parche de gestión de tiempo (freno de apertura, factores neutros de primer movimiento — primera jugada a 3+2: 19s → 6s) + fix del protocolo UCI ponder/infinite (sin fuga de bestmove en búsquedas auto-terminadas — el bug de Arena "congelado tras partida nueva") + bucle de comandos a prueba de fallos. ✅ (+14.3 ± 13.5 Elo SPRT vs v2.6.7, LOS 98.1%; **~2890 ± 25 CCRL**)
