...
**Current version: v2.8.0 — ~2975 ± 25 CCRL (SPRT pending)** (Syzygy endgame tablebases: exact results at five men or fewer, probed inside the search and used to filter the root moves by distance-to-zeroing so won endings are actually converted. Implemented as a managed port, verified against an independent prober over 3000 random endgames with zero discrepancies). See [CHANGELOG](CHANGELOG.md).
