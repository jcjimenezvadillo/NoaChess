...
## 2026-07-15 (v2.6.8) — time-management patch: bullet sustainability guard

**SPRT vs v2.6.7.1 (tc 10+0.1, non-regression bounds 0/10): non-regression confirmed, cut at 420 games [0.509], +6.9 ± 23.7 Elo, LOS 71.7%, DrawRatio 51.5%.** Fixes the Arena bullet death spiral: NoaChess played the opening fast, then slowed to 3-4s per move once the opening damp faded, bled 2-3s net per move against a 1s increment, and kept spending 1-2s per move (hard deadline ~4s!) with 5s on the clock — losing won games (+0.7, even forced mates) on time. Root cause: the reference formula folds 49 future increments into the usable time and its only brake is the 20%-of-remaining-clock cap, which lets the clock decay geometrically instead of stabilizing the per-move spend around the increment.

- time: sustainability guard (sudden-death branch only) — the soft target is bounded by `inc + clock/16` and the hard deadline by `inc + clock/4 - overhead`. Healthy clocks are untouched (the thresholds sit above the reference curve until the clock drops); in time trouble the spend converges to the increment and the engine physically cannot bet most of a 5-second clock on one move (2+1 with 5s left: hard deadline 3.96s -> 2.22s).
...
- tests: SustainabilityGuard pinned in both directions (time-trouble budgets bounded, healthy 90s middlegame budget unchanged) — 117 tests green.

**Development record — block 4H material imbalance (not shipped in v2.6.8):** **CUT by the block decision rule (like king-safety Fase B): SPRT a (raw reference diagonal): −30 Elo at 440 games [0.457]; SPRT b (texel bishop-pair diagonal restored): ±0 at 250 games [0.496].** The port itself was verified value-by-value against the reference material.cpp (tables, formula, /16 — exact; the arithmetic pinned by a hand-computed test). The failure is structural, and it is the recurring ×0.48-rule lesson: the imbalance tables were tuned inside the reference's eval ecosystem, while NoaChess's piece values are texel-tuned on its own games WITHOUT the imbalance term — the tuner already absorbed the average synergies (knight-with-pawns, rook redundancy...) into the piece values themselves, so adding the tables on top contributes zero-sum noise. The a/b pair proves it cleanly: the only part of the table NOT already absorbed (the bishop pair, which existed as an explicit term) is exactly the part whose removal cost −30. **Future rescue path (end-of-roadmap review, together with king-safety Fase B): re-run texel tuning of the piece values WITH the imbalance term active, so the tuner splits the work between them.** Implementation kept in git history (MaterialImbalance.cs with a piece-count-keyed direct-mapped cache, ~2.4% NPS); reverted from the branch, BishopPair S(44,68) restored.

Implementation record for the future rescue (all in git history on this branch):

- Tord Romstad's second-degree polynomial (material.cpp `imbalance()`): every PAIR of pieces scored — own-piece synergies (`QuadraticOurs`: knights gain with own pawns, second rook worth less, queen+rook redundant) and enemy interactions (`QuadraticTheirs`: queen strong vs rooks, knight good vs many pawns). Bishop pair = "extended piece" index 0. Tables in raw reference units; reference /16 then NoaChess ×0.48 → combined ×3/100 at the end; no re-centering (pure White−Black difference, exactly zero for symmetric material).
- perf pattern that worked: per-instance direct-mapped cache keyed by the packed ten piece counts (counts only change on captures/promotions) — pure eval cost ~2.4% NPS measured with an identical-tree control build.
