...
the shipping champion.** `fqmix` retrains the exact fq60 recipe with all three measured training
changes at once - no weight decay on the feature transformer (+11.1 alone), the reference-style
loss with its constants refit to this corpus by regression, offset 240 scale 145 (+11.9 alone),
and the lambda schedule 1.0 to 0.7 (+5.3 alone, unconcluded) - and the axes add rather than
overlap. Architecture unchanged, per-node cost identical, so the fixed-nodes verdict carries to
the clock as is. Over the fq60 that played until v5.0.x this stacks roughly +30 of training-side
Elo in one week, none of it costing a node per second.

The single-thread gauntlet scores **81.7% over 240 games** against the same 12-engine field:
**3342 +-86 CCRL**. The band is wide because the field is saturating - the engine beat the
strongest opponent in it (rated 3281) at 55% and six of the twelve at 90% or better - and a
saturated field resolves absolute figures poorly. The next calibration needs stronger opponents;
the precise instrument for the delta remains the SPRT's +19.6.

Also measured this cycle, both negative. The **psqt two-headed net** (the material half in an
int32 lane outside the quantized path, engine side shipped in v5.0.2.1) trained on the fqwd0
recipe and measured **-6.4 [-18.3, +5.5], H0 over 1,691 fixed-node games** against fqwd0; the
lane is verified exact to the centipawn against the trainer on the real net, so the verdict is
about content, not correctness. Its one cheap retry is a head on the fqmix recipe. The
reference's **all-depth shallow pruning ladder** measured H0 twice (-8.8 complete over 1,303
games; -13.7 with the futility rung removed, over 938) and stays in the tree as inert
off-by-default options (`PruningLadder`, `PruningLadderFutility`) with a named precondition for
any retry: port the full reference reduction curve first, because the ladder's constants assume
reductions this engine does not make.
...
kept alongside as rollbacks.


## 2026-08-27 (v5.1.0) - the net stops decaying its own transformer

**The first accepted H1 in weeks, and it ships.** Retraining the exact fq60 recipe with the
feature transformer's weight decay removed (`-FtWeightDecay 0`, arm `fqwd0`) measured
**+11.1 Elo [+2.3, +19.9], LLR +3.02, H1 over 3,224 games** at fixed nodes against the playing
net. The architecture is unchanged, so the per-node cost is identical and the fixed-nodes verdict
carries to the clock as is. The single-thread CCRL gauntlet anchors the absolute figure at
**3242 +-25** (72.9% over 240 games, 12-engine field), statistically indistinguishable from
fq60's 3271 +-40; the SPRT is the precise instrument for the delta. Generation ran on the same
machine at low priority during the gauntlet, which is recorded here rather than hidden.

The rest of the week's training campaign, measured one variable at a time on the same recipe:
the reference loss with its raw constants **lost -41.8 H0** (its offsets assume a different
centipawn scale; a refit against this corpus's own score/result pairs gives 240/145 and retrains
as `fqloss2`), and the lambda schedule 1.0 to 0.7 ended **+5.3 in 6,000 games without
concluding** - real but small, kept to stack onto the next winning recipe rather than shipped
alone. The corpus is being extended by 270M positions (to ~594M total), generated by the current
engine's search at the same 6,000-node label depth and with the same teacher net as the existing
324M, so volume stays the only moving axis, and the two-headed psqt net is built end to
end - loader, accumulator lane, finny cache, evaluator, trainer head with exact virtual-row
folding, export, verify_export reproduction and parity tests - waiting for its training slot.

Both bots run 5.1.0. Deployment verified by embedded-net hash on both platforms, with the
previous binaries kept alongside as rollbacks.


## 2026-08-23 (v5.0.2.1) - in a LOST position the tablebase filter left one move, and it was the wrong one

Reported from a real bot game with screenshots, and reproduced on the exact position:

```
4r3/1P1K4/8/8/6p1/q6k/8/8 w - - 0 83
```

The white king on d7 stands next to an undefended black rook on e8, and `Kxe8` also clears b8 for
the pawn. The engine played `Kc6`. Same binary, only the option changed:

```
tablebases, limit 7    Kc6    mate -6
no tablebases          Kxe8   mate -8
```

**With tablebases it is mated two moves sooner AND cannot even see the alternative**, because the
root filter has removed it from the move list.

`FilterRootMovesByTablebase` ranks root moves by DTZ and keeps a band around the best rank. The
band is `bestRank > MaxDtz / 2 ? (90 - halfmoveClock) / 8 : 0`, and that condition means "the
position is won". In a lost position the slack is **zero**, so the filter keeps exactly ONE move.
DTZ is the distance to the next IRREVERSIBLE move, so maximising it while losing means **refusing
to capture** - a capture zeroes the counter. Every line of that filter was written to protect a
WIN; applied to a loss it does the opposite of chess.

A root that is simply lost is now not filtered at all. `BlessedLoss` is what makes the distinction
safe: there the fifty-move rule genuinely can still save the game, so stretching DTZ is correct and
the filter keeps applying. A plain loss cannot be saved by anything, so there is nothing left to
protect and the search should be free to pick the most resilient practical try. Switching the
in-search tablebase scores off on the way out is not optional either: without it every losing
continuation would come back as the same flat number and the search would be as blind as the filter
was.

Verified: with tablebases at limit 7 the engine now plays `Kxe8` with **node counts identical to
the arm with no tablebases** (1,805,288). The two won-position guarantees the filter exists for are
unchanged - K+Q+Q vs K still mates in 3 without hanging a queen, and with the fifty-move counter
high it still pushes the pawn.

**`SyzygyProbeLimit` goes back to 7**, so v5.0.3's change is withdrawn. Two reasons, and the first
is a mistake worth naming: the 4.4x probe cost that justified dropping to 5 was measured with the
tables on THIS machine's mechanical drive, and the bot runs on a Mac with them on a PCIe SSD, where
that cost does not exist - the fix for one machine's disk was applied to another machine that never
had the problem. The bot's own record agrees: since 10 August the OPPONENT loses on time 45 times
and this engine twice, in about 700 games. Second, v5.0.3 was never measured head to head against
v5.0.1, so it did not clear the bar it should have.

Byte-identical to v5.0.2 on the shipped net with no tablebases (2,803,122 nodes over 150
positions), so this filter is the only behaviour that changes. 376 tests.
