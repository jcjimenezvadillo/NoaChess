...
**SPRT vs v2.6.7 (tc 10+0.1, non-regression bounds [-5, 5]): +14.3 ± 13.5 Elo, LOS 98.1%, H1 accepted, DrawRatio 44.1%** — the patch not only doesn't regress, it gains. **Strength: ~2890 ± 25 CCRL** — LTC gauntlet (tc=60+0.6, 448 games; 6 clean anchors 2863–2893, mean 2878, consistent with 2895+14 within error). Field audit: **KnightX-2932 EXCLUDED going forward** — three consecutive gauntlets anchor NoaChess 60–130 above every other opponent (2953 → 2970 → 3021, drifting), so its label is wrong (~2830 real). Pedone-2978 anchors low twice in a row (2841 → 2811, plays ~3050 real?) — on watch, one more run decides. Patch release targeting two Arena-observed problems: heavy clock use in the opening at short TC (1+0, 3+2) and a frozen engine after starting a new game (Ctrl+N + DEMO) without restarting it. No evaluation changes.
...
- uci: THE Arena freeze fix — a "go ponder" / "go infinite" search that finished on its own leaked its "bestmove" while the GUI still considered the search pending, which UCI forbids. The leak fired at the end of nearly every game: Arena's Permanent Brain ponders positions with forced mates, and a mate score breaks iterative deepening in milliseconds. The stray bestmove desyncs the GUI (Arena consumes it as the answer to the NEXT "go"), so the engine looks frozen from the next game on until restarted. Now a self-terminated ponder/infinite search parks on the cancellation handle and only answers when the GUI resolves it ("stop" -> bestmove; "ponderhit"/new position -> suppressed), the standard conforming behavior. Verified: no bestmove within 3s while pondering a mate-in-1, answer on stop, new game starts, ponderhit relaunches.
