...
| **5.9.2** (candidate) | **IN TRAINING: the complete fqcohuman, 60 of 60 epochs.** Seven epochs left after the 39 + 14 banked; the shipped 5.9.x net is epoch 14 of the second stage. Gate: SPRT at fixed nodes against the shipped net and against fqhuman, H1 or it does not ship. The run goes under two new loader guards (see CHANGELOG, unreleased): four runs in two days died to memory that read back wrong on the training box, reproduced outside the trainer as a single flipped bit that was gone on the next read. A memory test of the box is pending, on the user's call. | training |
