...
CCRL figure is v5.9.2's **3321 +/- 45**; v5.9.3 through v5.9.11 have not been regauntleted.

**Open right now (2026-09-19):** the off-by-default search options are being re-measured one at a
time against the v5.9.11 build at 100,000 fixed nodes, because every earlier fixed-node verdict was
taken with the `improving` flag half wrong (v5.9.11 entry in CHANGELOG). First verdict, `NmpEvalGate`
...
**`NmpEvalR` closed H1: +14.4 +/- 11.1, LLR +2.96 over 1,492 games against v5.9.12, and ships ON in v5.9.13** - the second H1, from the re-investigation's third-ranked retry.
**The package of the three slow positives closed H1: +11.8 +/- 9.4, LLR +3.00 over 2,259 games against v5.9.12, and ships ON in v5.9.14** - none of the three closed alone. `HindsightReset` with `LmrDeeperResearch` (the reference's configuration of the two): -7.1 over 1,025 (LLR -2.55), -4.7 +/- 13.3 over 1,108 (LLR -2.12) when the lane stopped; both options leave the engine in v5.9.15.
**`CheckExemptFutility` in its bounded form closed H1: +13.0 +/- 10.2, LLR +2.97 over 1,839 games against v5.9.13, and ships ON in v5.9.15** - the fourth H1 of the campaign and the second form of an option whose first form (every direct check to depth 4) read -5.7: a checking quiet is exempt from futility only at depth <= 2 and only when SEE >= 0. `TtCutoffNodeType` on the corrected labels, depth <= 4 and non-PV only: -6.6 +/- 14.7 over 923 (LLR -2.05) after -2.0 on the old labels; removed. `ReducedFutilityUnclamped` (the reference's own form, no depth gate, no floor): -1.7 over 1,203 against +3.8 clamped; removed, the clamped form stays for the next package. `HistoryPrune` at scale 49 with the count: -10.8 +/- 20.3 over 474 (LLR -1.47), cut; at 20 with the count it had read +3.0 over 2,015, so the last shape is the aggressive one, scale 8.
**`TtCutoffHistory` in its third form closed H1: +16.6 +/- 12.2, LLR +3.00 over 1,300 games against v5.9.14, and ships ON in v5.9.16** - the fifth H1 of the campaign, from a family whose first two forms read flat (+3.4) and H0 (-18.4): a quiet TT move failing high at a non-PV node earns about 0.56x of a searched cutoff's bonus, capped at depth 6. Razoring's fifth shape, every earlier gap corrected: -13.0 +/- 21.3 over 406 (LLR -1.52), removed, the line closed. The package of the three remaining slow positives (`CutNodeLmr`, `ReducedFutility`, the quiescence evasion prune with its exemption) read -5.0 +/- 16.8 over 724 (LLR -1.35) against v5.9.14: they do not add, cut. `CutoffCountLmrAllNode` passed the 1,200-game screen at +6.7 and settled flat, +4.5 +/- 12.3 over 1,341 (LLR -0.12); a tie, kept off.
...
count -4.0%: a saving instead of a swap). `CorrectionWeightCap`: **flat, -1.4 +/- 12.2 over 1,209**.
The correction family is closed after three shapes - the reference's weights flat, its integrator
H0, gentler updates flat - and the two options left the engine. At the 1,200-game screen against
v5.9.12: `ReducedFutility` (parent futility on the reduced depth with history, the reference's raw
constants, depth <= 8, floored at -1) **+3.8 +/- 12.4, LLR -0.30** and the quiescence evasion prune
with its exemption **+4.6 +/- 12.2, LLR -0.10**, both slow positives, cut; `CutNodeLmr` on the
corrected labels **+9.5, LLR +1.18** and the package of the three slow positives (`TtNoPvCutoff`,
`FutilityFailSoft`, `ImprovingAboveBeta`) **+10.7, LLR +1.24** continue to a verdict. `CutNodeLmr`
settled at **+7.4 +/- 9.5 over 2,011, LLR +1.04**: a slow positive where it had been H0 three times
on the old labels (-10.7 the last), which is what the label fix predicted; cut at 2,000 and kept
for the end package. The fidelity bundle without the pawn-endgame guard read **flat, +1.7 over
1,211** against -3.8 with it, so `PruneNpmGuard` cost about five Elo alone and was removed; the
...
|---------|----------|--------|
| **5.9.16** | **SHIPPED 2026-09-22: a transposition cutoff teaches the move that caused it.** A node cut by the transposition table returned at once and its quiet move, the one that had failed high, learned nothing; only searched cutoffs fed the history tables. `TtCutoffHistory` ON in its third form: a quiet transposition move failing high at a non-PV node earns about 0.56x of a searched cutoff's depth-squared bonus in the butterfly and first continuation tables, capped at depth 6 as the reference caps `min(112 * depth, 695)`. **Fixed-node SPRT at 100,000 nodes against v5.9.14: +16.6 +/- 12.2, LLR +3.00, H1 over 1,300 games (LOS 99.6%)**; the full uncapped bonus had read +3.4 and the reference's malus -18.4 H0. Razoring removed again after a fifth, fully corrected shape (-13.0 over 406), the line closed. Node-identical otherwise (60 positions at depth 11, 4,902,968 nodes against v5.9.15 with the option on). CI node count regenerated. 461 tests. | shipped |
| **5.9.15** | **SHIPPED 2026-09-22: a checking quiet is no longer pruned blind at the leaves.** Futility pruning dropped quiets on the static eval alone and this engine has no gives-check test before the make, so a quiet check was pruned like any other move whenever the eval sat under alpha; the reference never futility-prunes a checking quiet. The first form (every direct check exempt to depth 4) measured -5.7 over 1,220 with +16.9% nodes. `CheckExemptFutility` ON in the bounded form, direct checks at depth <= 2 with SEE >= 0: **fixed-node SPRT at 100,000 nodes against v5.9.13, +13.0 +/- 10.2, LLR +2.97, H1 over 1,839 games (LOS 99.4%)**. Node-identical otherwise (60 positions at depth 11, 4,622,672 nodes against v5.9.14 with the option on): `LmrDeeperResearch` (H0 -4.4), `HindsightReset` (-3.0 alone, -4.7 with the deeper re-search), `ReducedFutilityUnclamped` (-1.7) and `TtCutoffNodeType` (-2.0, then -6.6 on the corrected labels) removed with tombstones. CI node count regenerated. 461 tests. | shipped |
| **5.9.14** | **SHIPPED 2026-09-22: three slow positives that close together.** `TtNoPvCutoff` (no transposition cutoff at PV nodes, +6.4 over 2,920 alone), `FutilityFailSoft` (the futility-pruned quiets' value raises a fail-low node's bound, reference step 14, +7.8 over 2,286 alone) and `ImprovingAboveBeta` (improving |= eval >= beta after the null move, +5.5 over 1,205 alone) ON: **together +11.8 +/- 9.4, LLR +3.00, H1 over 2,259 fixed-node games against v5.9.12**. The KQK test `ABareKingTakesTheLongestRoadToMate` forbade Ka8 as the fastest loss; an independent brute-force mate solver says Kc8, Ka8 and Ka7 are all mated in 4 and only Kc7 in 3, so it now forbids Kc7. Node-identical otherwise: `CaptureFutility` removed after two forms (flat -1.7, flat -2.9), `PruneNpmGuard` removed (about -5 inside a bundle), `ReducedFutilityUnclamped` added for measurement. 461 tests. | shipped |
| **5.9.13** | **SHIPPED 2026-09-22: the null move reduces more the further the eval sits above beta.** `NmpEvalR` ON: the null-move reduction `3 + depth / 4` gains `min((eval - beta) / 81, 3)` plies (the reference's `(eval - beta) / 168` in its units, x0.48; capped at three because verification starts at depth 14), on the TT-refined eval, reading no cutNode. **Fixed-node SPRT at 100,000 nodes against v5.9.12: +14.4 +/- 11.1, LLR +2.96, H1 over 1,492 games (LOS 99.5%)**; bench -1.0% nodes. From the re-investigation of the discarded null-move options: `NmpGateDeepR` had licensed `R = 7 + depth / 3` only on a set reverse futility had already cut. Node-identical otherwise: `LmpCountsPruned` H0 (-17.4 over 680) and removed, kept as `HistoryPruneCounts`; `CorrectionGravity` (H0 -9.6) and `CorrectionWeightCap` (flat -1.4) removed, the correction family closed. Publish script fixed (it checked for the embedded network before `isready`). 461 tests. | shipped |
