...
generations on an expanding corpus (fqwd0 -> fqmix -> fq594 -> fqhuman -> fqcohuman -> fqcohuman3 ->
the currently embedded `fqco592`; full detail in [NNUE_HISTORY.md](NNUE_HISTORY.md)), a line-by-line audit of
...
CCRL figure is v5.9.2's **3321 +/- 45**; v5.9.3 through v5.9.16 have not been regauntleted.
...
because every earlier fixed-node verdict was taken with the `improving` flag half wrong (v5.9.11 entry in CHANGELOG). First verdict, `NmpEvalGate`
...
come out. **[RESOLVED 2026-09-22: the third shape measured was `NmpBelowBetaGate` (null probes only
where eval >= beta - 25), -3.5 over 1,217, so the entry gate is closed; the eval-margin reduction
`NmpEvalR` ships in v5.9.13 instead, +14.4 H1.]** Reading the code of the other 2026-09-08 H0 items against the reference produced two
...
back **flat: -0.4 +/- 11.0, 0.498 over 1,674 games, LLR -1.71, cut by decision** - with the flag
...
its site. **[Razoring came back once more: a fifth shape with every earlier gap corrected read -13.0
over 409 and was removed again in v5.9.16, the line closed.]** `CutoffCountLmrRef`, the reference's own shape
...
cutoff earns the butterfly and continuation bonus a searched cutoff earns): **flat, +3.2 +/- 12.9,
0.506 over 1,218 games, LLR -0.42, cut by decision at the 1,200-game screen**. The second form adds
...
sign against it, -2.0 +/- 13.4, 0.498 over 1,206 games, LLR -1.49, cut by decision at the screen**;
...
+0.9 +/- 12.9, 0.500 over 1,218 games, LLR -0.95, cut by decision at the screen**; the same verdict
...
the problem. **[REOPENED 2026-09-21/22: `TtCutoffNodeType` came back on the corrected labels (-6.6
over 923) and was removed in v5.9.15; `TtCutoffHistory` came back in a third form, without the malus,
and ships ON in v5.9.16 (+16.6, H1 over 1,300 games); `TtNoPvCutoff` ships ON inside v5.9.14's
package.]** `CaptureFutility` in its reformed shape (raw margins 234 + 247 per ply plus the
victim and the capture history, checking captures exempt, full depth in the margin): **flat with the
sign against it, -2.0 +/- 13.0, 0.497 over 1,207 games, LLR -1.58, cut at the screen**. Second
form queued: the reference's REDUCED depth in the margin and its lmrDepth < 8 gate (the full depth
only widens the margin, so the first form pruned less than the reference does). **[Measured: flat,
-2.9 +/- 12.8 over 1,206; removed in v5.9.14.]** `CheckExemptFutility`
(a quiet that gives direct check is not futility-pruned; +16.9% nodes): **-4.9 +/- 13.0, 0.492 over
1,223 games, LLR -2.26, 77% of the way to H0, cut at the screen** - the tactics it keeps do not pay
...
is pruned after all. **[Measured: the bounded form, depth <= 2 and SEE >= 0, closed H1 at +13.0 and
ships ON in v5.9.15.]** `CorrectionBlend` (the reference's key set and relative weights on the set's
own budget) read +4.9 at 1,214 games and settled **flat: +1.7 +/- 9.9, 0.502 over 2,014 games, LLR
-1.28, cut by decision**. Its other conceivable form, the reference's write-side rates, is the same
...
worst 5% of late quiets): **flat, -3.3 over 1,187 games, cut at the
screen**. `FutilityFailSoft` alone: **+8.3 +/- 9.4, 0.511 over 2,293 games, LLR +1.45, positive
...
`HindsightReset`: **flat, -3.0 +/- 13.3, 0.496 over 1,073 games, LLR -1.73, cut at the screen** -
re-applying the adjustment on re-searches was harmless; the second form pairs the reset with the
reference's own thresholds, which this engine's hindsight shape never used. **[Measured: the reset
with `LmrDeeperResearch`, -4.7 over 1,108; both removed in v5.9.15.]**
The package (`TtNoPvCutoff` + `FutilityFailSoft`) read +12.4 at 1,202 games and settled at **+5.3
+/- 9.6 over 2,251 (LLR +0.10)**: the two do not add, and it was cut as a slow positive.
`ImprovingAboveBeta` (improving |= eval >= beta after the null move): **+5.8 +/- 12.9 over 1,206,
LLR +0.18**, the same kind of slow positive. Both go into the end-of-campaign package.
...
together): **H0, -4.0 +/- 10.8, LLR -2.96 over 1,729** - four real defects fixed at once cost a
little; the prime suspect is the pawn-endgame guard, which switches all shallow pruning off in king
and pawn positions, so the bundle is re-run without it. **[Re-run below: flat, +1.2 over 1,213.]**
...
From here every candidate is measured against v5.9.12. `LmpCountsPruned` (quiets pruned by history
or SEE count toward the LMP budget, as the reference's moveCount counts every legal move): **H0,
-17.4 +/- 16.9 over 680** - counting the SEE-pruned quiets fires LMP far earlier than thresholds
calibrated on searched quiets; removed, and its one useful part kept as `HistoryPruneCounts`, which
counts only the quiets `HistoryPrune` removes (the history prune alone +1.8% bench nodes, with the
count -4.0%: a saving instead of a swap). `CorrectionWeightCap`: **flat, -1.2 +/- 12.5 over 1,209**.
The correction family is closed after three shapes - the reference's weights flat, its integrator
H0, gentler updates flat - and the two options left the engine.
**`NmpEvalR` closed H1: +14.4 +/- 11.1, LLR +2.96 over 1,492 games against v5.9.12, and ships ON in
v5.9.13** - the second H1, from the re-investigation's third-ranked retry. At the 1,200-game screen
against v5.9.12: `ReducedFutility` (parent futility on the reduced depth with history, the reference's raw
constants, depth <= 8, floored at -1) **+4.8 +/- 13.1 over 1,180** and the quiescence evasion prune
with its exemption **+4.6 +/- 12.5 over 1,201, LLR -0.09**, both slow positives, cut; `CutNodeLmr` on the
corrected labels **+9.5, LLR +1.18** and the package of the three slow positives (`TtNoPvCutoff`,
`FutilityFailSoft`, `ImprovingAboveBeta`) **+10.7, LLR +1.24** (final: +11.8 over 2,259, H1) continue to a verdict. `CutNodeLmr`
settled at **+7.3 +/- 9.8 over 2,012**: a slow positive where it had been H0 three times
on the old labels (-10.7 the last), which is what the label fix predicted; cut at 2,000 and kept
for the end package. The fidelity bundle without the pawn-endgame guard read **flat, +1.2 over
1,213** against -4.0 with it, so `PruneNpmGuard` cost about five Elo alone and was removed; the
other three fixes measure level and stay off as a tie. Queued behind them: the second hindsight
...
`CaptureFutility` and `CheckExemptFutility`, and the two quiescence correction keys. The ladder with
its term restored was dropped from that queue by the re-investigation: its earlier forms carried the
reference's units, and its one valuable rung, parent futility on the reduced depth, was measured alone
as `ReducedFutility`; the option stays off, and its stopped run on the dead-term build read 0.485
over 335. Every other item has since been measured; the verdicts are above, in what follows and in
the table below.
**The package of the three slow positives closed H1: +11.8 +/- 9.4, LLR +3.00 over 2,259 games against v5.9.12, and ships ON in v5.9.14** - none of the three closed alone. `HindsightReset` with `LmrDeeperResearch` (the reference's configuration of the two): -7.1 over 1,025 (LLR -2.55), -4.7 +/- 13.3 over 1,108 (LLR -2.12) when the lane stopped; both options leave the engine in v5.9.15. `LmrDeeperResearch` alone was never re-measured on the corrected `improving` flag: its H0, -4.4 +/- 10.9 over 1,744, dates from 2026-09-08.
**`CheckExemptFutility` in its bounded form closed H1: +13.0 +/- 10.2, LLR +2.97 over 1,839 games against v5.9.13, and ships ON in v5.9.15** - the fourth H1 of the campaign and the second form of an option whose first form (every direct check to depth 4) read -4.9: a checking quiet is exempt from futility only at depth <= 2 and only when SEE >= 0. `TtCutoffNodeType` on the corrected labels, depth <= 4 and non-PV only: -6.6 +/- 14.7 over 923 (LLR -2.05) after -2.0 on the old labels; removed. `ReducedFutilityUnclamped` (the reference's own form, no depth gate, no floor): -1.7 over 1,203 against +4.8 clamped; removed, and the clamped form went into the next package. `HistoryPrune` at scale 49 with the count: -10.8 +/- 20.3 over 477 (LLR -1.47), cut; at 20 with the count it had read +3.0 over 2,015, so the last shape is the aggressive one, scale 8.
**`TtCutoffHistory` in its third form closed H1: +16.6 +/- 12.2, LLR +3.00 over 1,300 games against v5.9.14, and ships ON in v5.9.16** - the fifth H1 of the campaign, from a family whose first two forms read flat (+3.2) and H0 (-18.4): a quiet TT move failing high at a non-PV node earns about 0.56x of a searched cutoff's bonus, capped at depth 6. Razoring's fifth shape, every earlier gap corrected: -13.0 +/- 21.3 over 409 (LLR -1.52), removed, the line closed. The package of the three remaining slow positives (`CutNodeLmr`, `ReducedFutility`, the quiescence evasion prune with its exemption) read -5.0 +/- 16.8 over 741 (LLR -1.35) against v5.9.14: they do not add, cut. `CutoffCountLmrAllNode` passed the 1,200-game screen at +6.7 and settled flat, +2.1 +/- 12.1 over 1,367 (LLR -0.78); a tie, kept off.
**After v5.9.16 (2026-09-22).** `FailHighDamping` (the reference's fail-high damping, main search) settled flat, +1.2 +/- 9.7, LLR -1.53 over 2,017 games, and its second form with the quiescence half (`FailHighDampingQs`) read -8.7 +/- 18.3, LLR -1.56 over 606: the family is exhausted and both options leave the engine in the next build. `NmpBelowBetaGate` (null probes only where eval >= beta - 25): -3.5 +/- 12.7, LLR -2.02 over 1,217 against v5.9.15, so the null-move entry gate is closed after three shapes (`NmpEvalGate` H0, `NmpGateDeepR` flat, this one) and `NmpBelowBetaGate` with `NmpGateMargin` leaves the engine in the next build. The two quiescence correction keys read flat, `QsContCorrection` +2.6 +/- 12.5 over 1,214 against v5.9.15 and `QsEntryKey` 0.0 +/- 13.1 over 1,160: ties, kept off. `TtRule50Guard` is node-identical below a halfmove clock of 96, so a fixed-node SPRT cannot see it; it stays off pending a decision. `HistoryPrune` at scale 8 with the count, its last shape, was resumed twice and pooled: +2.7 [-4.9, +10.2] over 3,230 games, LLR -1.56, a tie, so the option stays off instead of leaving the engine. Its tie is the SCALE 8 shape with `HistoryPruneCounts` on, not the shipping defaults (scale 20, counts off), which read -3.3 over 1,187. [RESOLVED 2026-09-23, verdicts below:] the four candidates then running (`NmpPackage`, `PvWindowEarly`, `CutNodeLmr` and the quiescence evasion prune on the new base) and v5.9.16 against v5.9.11 all closed, and so did the review of the nine off options whose only verdicts, from 2026-09-08/09, were taken with the `improving` flag half wrong (`PriorFailLowBonus`, `RfpTtMoveGuard`, `LmpCountAllMoves`, `DrawRandom`, `ProbCutAllowNull`, `SingularTight`, `CaptureSeePruneDeep`, `TtMateReuse`, `NmpNonPvOnly`), of `MoveCountLmr`, of the reference's singular quiet LMR adjustment and of the check extension dropped in v2.8.2. Five of the nine are decided and leave the engine in the next build: `TtMateReuse`, whose behaviour gate over 30 middlegame forced mates came out flat (median nodes to the final mate distance -3.2%, 16 positions faster and 12 slower, one mate lost and the reported distance changed in five of the thirty), so the -3.0 +/- 10.2 H0 over 2,091 stands; `RfpTtMoveGuard`, -11.1 +/- 14.3 over 1,002, H0 (the reference pairs that guard with a margin a third of ours that runs to depth 18, while ours asks 85 a ply to depth 6 and cuts only the clear cases, so the guard only gave up sound cuts); `LmpCountAllMoves`, -6.0 +/- 11.9 over 1,446, H0, with `LmpCountsPruned` at -17.4 +/- 16.9 over 680, so both ways of counting more moves toward the same threshold lost; `ProbCutAllowNull`, +0.7 +/- 7.4 over 3,892, H0, a probe the reference's own child never makes because its stand-pat already sits below its beta; and `DrawRandom`, whose first form measured -0.1 over 3,522 pooled games and whose corrected form was gated out before an SPRT (the draws it would flatten are 2.9% of the jittered returns, and its bench differed from the first form's in 11 positions of 60 with no best move changed).

**2026-09-23: v5.9.16 confirmed against v5.9.11, and the nine old off options re-measured.** The confirmation run asked what the campaign is worth end to end: v5.9.16 against v5.9.11 at 100,000 fixed nodes with the same embedded net on both sides, **+25.3 +/- 16.3, LLR +2.97, H1 over 728 games (178-125-425, LOS 99.9%)** - the measured value of the five H1 of 2026-09-22 together (`GoodCaptureSlack` in v5.9.12, `NmpEvalR` in v5.9.13, the package of three in v5.9.14, `CheckExemptFutility` in v5.9.15, `TtCutoffHistory` in v5.9.16). The nine off options whose only verdicts dated from 2026-09-08, taken before the `improving` flag was fixed in v5.9.11 and before the cutNode labels were corrected on 2026-09-21, and the LMR adjusters that had only ever been measured inside a package, were then re-investigated the way the second sweep was run: one reader per pair of features and an adversarial second opinion, every diagnosis re-verified against the code and the reference, implementation behind off switches that leave the default search node-identical (60 positions at depth 11, 4,902,968 nodes), an adversarial review of every change, and one fixed-node SPRT each at 100,000 nodes, elo0=0 elo1=10. Deleted from the engine in the working tree, so they ship with the next build: `ReducedFutility` with `ReducedFutilityRefDepth`, whose third arm on the reference's own pre-adjuster reduced depth (its log-product table on the move count, 0.385x when not improving, +982 and +929 at ttPv) read **H0, -12.7 +/- 15.2 over 792 games, LLR -2.95**, which with the clamped arm (+4.8 over 1,180, never closing) and the unclamped one (-1.7 over 1,203) exhausts the line; `MoveCountLmr`, the reference's base-offset + moveCount pair, H0 with the reference's own base (-7.4 over 1,400 on 2026-08-30) and **-7.9 +/- 16.1 over 757, LLR -1.92**, with the base recalibrated node-neutral (1347, inside 0.4% at depth 12); `LmrChecks`, a new option because the reference has no gives-check exclusion in its reduction, **-3.6 +/- 12.7 over 1,199, LLR -2.05** - about 2% of reduced moves are checks and the bench loses 2.0% of its nodes, so the saving is real and the play is worse; `NmpPackage`, the reference's whole null-move subsystem as one package, H0 -23.2 over 614 on the raw gate on 2026-09-01 and **flat, -0.3 +/- 13.1 over 1,119** on the corrected x0.48 gate; and the quiescence evasion prune with its exemption, **-6.4 +/- 13.0 over 1,205, LLR -2.6** against v5.9.16 after its +4.6 screen over 1,201. With `FailHighDamping` and its quiescence half, `NmpBelowBetaGate` with `NmpGateMargin` and the five of the nine decided above, that is the deletion list for the next build.

**The rest came back ties (2026-09-23).** `PriorFailLowBonus` in a second form, the reference's step-20 terms complete with the amounts re-expressed in this engine's depth-squared currency after the first form deposited 21x a cutoff's bonus and cost +20% nodes: **flat, +3.2 +/- 12.6 over 1,200 games**, against -22.2 over 565 for the first. `SingularTight` in a second form, the reference's whole singular block (depth 6 plus one at ttPv, margin (28 + 32 at ttPv non-PV) * depth / 63, the multi-cut return, negative extensions -3 and -2, the pseudo-legality and shuffling guards): **flat, +3.9 +/- 9.7 over 1,952**, against -0.3 H0 for the first. `CaptureSeePruneDeep` in a second form (margin 85 a ply plus the capture history, the loss guard and the last-piece exemption, no alpha guard) read +10.8 at 1,280 games and settled at **+3.9 +/- 7.5 over 3,214**, against -1.7 H0 over 2,610. `NmpNonPvOnly` re-measured **flat, +2.1 +/- 13.3 over 1,190**, and its last form, the reference's own node condition (`NmpCutNodeOnly`, a null probe only at expected cut nodes, -9.1% bench nodes when on, added for this measurement): **flat, +2.2 +/- 12.6 over 1,149**. `CutNodeLmr` on the new base is **flat, -0.6 +/- 12.7 over 1,169**, where it had read +7.3 over 2,012 against v5.9.12 before the five H1 shipped, and its contingency arm `CutNodeLmrTtPv` (the reference's full ttPv treatment at cut nodes, 3.7% of reduced moves), also new, is **flat, +1.1 +/- 10.1 over 1,877**. `HistoryPrune` at scale 8 with its count was resumed and pooled over three runs: **+2.7 [-4.9, +10.2] over 3,230 games, LLR -1.56**; with the earlier forms (-3.3 over 1,187, +3.0 over 2,015, -10.8 over 477) the family is a tie, so it stays off instead of leaving the engine. Two packages then asked whether the ties add, and they do not: `PriorFailLowBonus` + `SingularTight` + `CaptureSeePruneDeep` + `PvWindowEarly` read **-3.5 +/- 13.1 over 1,216, LLR -1.91, cut**, and `HistoryPrune` with its count + `CutoffCountLmrAllNode` + `NmpCutNodeOnly` + the fidelity trio read **flat, +2.9 +/- 12.5 over 1,222**. **Decided 2026-09-23 (v5.9.18): four of the fourteen ties ship, on judgment rather than Elo.** The
fidelity trio (`PruneLossGuard` + `LosingCaptureOrder` + `SmallProbCutExact`) and `PvWindowEarly`
each correct a real defect the port had against the reference, not a bet on a new heuristic, and
each costs zero or less: the trio measures **-1.42% bench nodes** on the shipping v5.9.17 build
(5,193,384 against 5,268,200 at depth 11, 60 positions), `PvWindowEarly` costs 3 nodes on 5.27
million. Node-identical against the manually-flagged v5.9.17 build (5,193,386 both ways). The
standing rule is that a tie can enter on the user's own delegated judgment given measured zero cost
plus a behavioural correction, and that is the case made here; neither reading is an H1. The other
ten stay off, pending a decision, one by one: `HistoryPrune` (+2.7 over 3,230), `CutoffCountLmrAllNode` (+2.1 over 1,367), `QsContCorrection` (+2.6 over 1,214), `QsEntryKey` (0.0 over 1,160), `PriorFailLowBonus` (+3.2 over 1,200), `SingularTight` (+3.9 over 1,952), `CaptureSeePruneDeep` (+3.9 over 3,214), `NmpNonPvOnly` (+2.1 over 1,190), `NmpCutNodeOnly` (+2.2 over 1,149) and `CutNodeLmr` with `CutNodeLmrTtPv` (-0.6 over 1,169, +1.1 over 1,877), plus `TtRule50Guard`, which a fixed-node SPRT cannot see. None of the ten corrects a known defect, and several (`HistoryPrune`, `CaptureSeePruneDeep`, `CutoffCountLmrAllNode`, `NmpCutNodeOnly`) had their point estimate shrink toward zero as more games ran - the signature of noise regressing to the mean, an argument against buying more games rather than for it. Tests 461 green throughout and the default search node-identical to v5.9.16 after every change; the CCRL figure is still v5.9.2's 3321 +/- 45, because a clock gauntlet needs the box free and the training has it. The Lichess bot on the Mac has run v5.9.16 since 2026-09-22 (hash verified after the deploy); the Windows bot carries the same binary and stays stopped, as only the Mac bot plays.

Larger open items that a fixed-node SPRT cannot judge: the PSQT-head retry on the fqco592 recipe (a
GPU run, explicitly open in the deep analysis - and, read in the code on 2026-09-20, not a launch
away: the trainer and the exporter both refuse a psqt head together with the coarse threat lane
every champion since fqcoarse carries, so the retry first needs psqt+coarse support end to end,
trainer to engine; an abandoned from-scratch attempt without the lane, `fqpsqt2`, sits at 15 of 60
epochs from 2026-09-05), `TimeScale` and the ponder options (need a clock match on a quiet box).
Syzygy WDL relabelling in the datagen, "still not wired" on the deep analysis page since v3.0.0, is
wired as of 2026-09-19: `NoaChess.DataGen --tb-path <dirs>` labels every recorded position inside
the tablebase range with the exact outcome under best play instead of the game's eventual result
(the search probes the same tables, so the score label in that range is the deployed engine's), off
unless the path is given, count and setting in every shard manifest. datascale5 was already running
on the previous build, so the first corpus to carry it is the next one. datascale5 itself completed
on 2026-09-22 (297,839,853 records in 62 shards from 33,927,604 games, teacher fqco592, root
diversity on, audited PASS). `fqco5911`, the fqco592 recipe warm-started from fqco592 on
datascale2 + datascale4 + datascale5 + selfplay-gen8 + the 17 elite label-book shards, is training
(epoch 6 of 7, due to finish in the middle of the morning of 2026-09-23); a fixed-node SPRT against fqco592 comes
next.
...
|---------|----------|--------|
| **5.9.21** | **SHIPPED 2026-09-25: a helper that misses the stop is no longer lost for the rest of the game.** Found on the Windows bot (Ryzen, Threads 24) on the first day of its clean-machine test against the older Mac: Task Manager showed the machine at 17-35% CPU while the bot played. Measured: a freshly started engine used 24 busy threads, about 26 cores, 83% of the machine, but in endgames it fell to 0.1-2 cores, threads in Wait/PageIn and Wait/Executive, nps collapsing from the usual ~1M to 38k. Cause in the host: the bot's Syzygy tables (151 GB, 6-man) lived on a mechanical disk, the engine memory-maps the table files, so every probe inside the search page-faulted on the disk and 24 threads issuing random reads saturated it; a helper only checks the stop every 2048 nodes, so helpers deep in tablebase territory could not honour a stop in time. The engine defect that made it permanent: since v5.9.3 a helper not back within 3000 ms of the stop is quarantined (the watchdog added after the 74-minute freeze), and the quarantine was FOREVER. In one game (11:26) eleven helpers were quarantined at once and the engine played the rest of the game on 13 of its 24 threads (per-thread CPU confirmed those eleven parked); 30 groups of cancellation/quarantine events over the day, almost all with tablebase hits in the last search, against 2 overnight with nothing else running. **Fix (`WorkerLoop`, `FindBestMoveParallel`, `RebuildPool`): a quarantined helper that finally returns rejoins the pool and is counted and woken by the next search; its late result, computed on an old position, is discarded instead of being written into `_workerResults`, the array every search reuses and votes from** (before, a stale move could land in the array of the search running when the helper came back, and the vote only admits helpers at least as deep as the main worker, which a long-running stale search could be). Count, reset and release of each search now happen under the same lock as quarantine and rejoin, so a concurrently rejoining helper cannot be released without being counted (the v5.9.4 crash class: a signal on a countdown already at zero); a pool generation makes a thread from a replaced pool (Threads changed) exit instead of indexing the new arrays; new diagnostic "helper thread N returned X ms after the stop and rejoined the pool". A helper that truly never returns is still never awaited. New test `HelperQuarantineTests`: a helper held back 4.5 s is quarantined, returns during a second search on a position where no move of the first is legal, rejoins, and the third search runs with it, paying no watchdog wait. Shipped on judgment, not by SPRT: a verified defect corrected, zero cost at one thread, the multi-thread path only changes when a helper is late. Single-thread search untouched (CI node count 127139, unchanged); net unchanged (fqco5912). Host configuration, not code: the WDL tables (68.2 GB, every probe inside the search) plus the DTZ tables up to 5 men copied to the SSD, SyzygyPath listing the SSD folder first and the mechanical disk second (each file comes from the first folder that has it; `;`-separated folders were already supported), so only the 6-man DTZ files, read once per move at the root, still come from the mechanical disk. 462 tests (128 Core + 334 Engine). Published Windows + Mac; Mac bot stopped for the Windows test (binary copied, not started), Windows bot restarts with 5.9.21 at the end of its game in progress. Gauntlet not run for this version. | shipped |
| **5.9.20** | **SHIPPED 2026-09-24: fqco5912 embedded, a real anneal on the current corpus.** The schedule experiment the 2026-09-23 investigation named as the strongest bet for the next net: the two largest NNUE wins (gen9 +18, fqcohuman3 +21.6) were the tail of one 60-epoch cosine anneal, while fqco592 and fqco5911 trained 7 epochs at a tiny lr range. fqco592's checkpoint gets a fresh 14-epoch cosine from lr 1.371433e-4 (the original schedule at epoch 46) to 1e-5 on the fqco5911 corpus (263 files, 1,219,446,812 records), otherwise identical hyperparameters. The run was terminated from outside at epoch 11 and resumed from epoch 10 at lr 3.3935e-05 (4 epochs left, lambda 0.710 to 0.7); final validation 0.005540 against fqco592's 0.005860 (the split leaks a little, so it reads optimistic). **Fixed-node SPRT at 100,000 nodes against fqco592, the same 5.9.19 binary on both sides: +34.3 +/- 19.7, LOS 100%, LLR 2.95, H1 over 477 games** (119-72-286); H1 closed early, so the estimate is likely inflated, against +7.8 +/- 6.0 over 5,401 games for the data step fqco5911. Supports the learning-rate schedule, not data volume, as the lever; a longer measurement against fqco5911 and a gauntlet are still to come. `nodecount_ref.txt` regenerated 127104 -> 127139 (the 5.9.19 binary already gave 127139). Search code untouched. 461 tests (128 Core + 333 Engine). Windows bot binary copied, bot not running (clean-machine test from 2026-09-25, Mac bot stopped for it). Gauntlet pending. | shipped |
| **5.9.19** | **SHIPPED 2026-09-23: a tablebase draw was invisible to almost the whole search.** Found reviewing two real bot games the user flagged as playing without a plan in a long endgame: one of them reached a King+Rook vs King+Rook+Bishop ending, no pawns, confirmed a draw by probing Syzygy directly (not through any engine's own search); the shipped engine, with the same tables loaded, stayed at -122 to -125 cp for 24 iterations straight, as if losing. Cause: the node-level Syzygy probe only fires when the fifty-move clock reads exactly zero or the root is already lost, a guard the WIN case needs (the rule can only turn a win into a draw) but a draw or a loss never does; in a long, quiet, capture-free stretch the clock is almost never zero, so the probe skipped nearly every node in the tablebase-covered subtree and NNUE evaluation, which does not know this material shape, took over. `TbDrawProbeAlways` ON: **verified on three separate points of the real game, each wrong (-122 to -125 cp) with the guard as shipped and correct (0 cp) with it loosened**, and on two won positions with unrelated material where the option changes nothing. Node-identical with Syzygy loaded on the 60-position bench at depth 11 (5,193,386 nodes in every configuration): the gate only narrows tablebase-size endgames a normal-play bench never reaches. Shipped on judgment (zero cost, a real defect corrected), not by SPRT; an SPRT was left running for a secondary read. The root's own move choice was never in danger - a separate, unconditional filter always restricts the root to game-theoretically optimal moves regardless of the clock, which is why the flagged game still drew - what this fixes is the search staying truthfully anchored to that draw instead of playing convinced it was losing. Gauntlet on field 2, single-threaded, the same field as v5.3.0 to v5.9.2: **57.7% over 313 games, 3364 CCRL +/- 40**, the highest of the series (previous best v5.8.2's 3353, the two bands overlap), against v5.9.2's 3321. The secondary SPRT for `TbDrawProbeAlways` closed flat at 2000 games ([0.500], LLR ~0), as expected for a fix targeting a narrow tablebase-only blind spot rather than general play. 461 tests. | shipped |
| **5.9.18** | **SHIPPED 2026-09-23: three fidelity fixes ship on judgment, not on Elo.** `PruneLossGuard`, `LosingCaptureOrder`, `SmallProbCutExact` and `PvWindowEarly` ON: none closed an SPRT (fidelity trio flat +1.2 +/- 10.1 over 1,213; `PvWindowEarly` flat +4.9 +/- 12.4 over 1,207, both against v5.9.15), but each corrects a real defect the port had against the reference rather than betting on a new heuristic, and each measures zero cost or better: the fidelity trio is **-1.42% bench nodes** (5,193,384 against 5,268,200 at depth 11), `PvWindowEarly` costs 3 nodes on 5.27 million. Shipped under the standing rule that a tie can enter on judgment given measured zero cost plus a behavioural correction. Node-identical against the manually-flagged v5.9.17 build. The other 12 options closed as ties this campaign stay off; several showed their point estimate shrink toward zero with more games, arguing against further measurement. 461 tests. | shipped |
| **5.9.17** | **SHIPPED 2026-09-23: fqco5911 embedded, a real but small data step.** Same recipe as fqco592, warm-started from its `.partial`, on a 1.3475x larger corpus (datascale2 + datascale4 + datascale5 + selfplay-gen8 + 17 elite WDL shards, 1,219,446,812 records against 904,912,411). **Fixed-node SPRT at 100,000 nodes against fqco592: +7.8 +/- 6.0, LOS 99.4%, LLR 2.96, H1 over 5,401 games**; the point estimate held inside [+4.1, +8.0] from game 800 to game 5,401. Also closed: v5.9.16 confirmed against v5.9.11 (+25.3 +/- 16.3, LLR +2.97, H1 over 728 games, same embedded net both sides), and a full re-investigation of nine off options measured 2026-09-08 with the `improving` flag half wrong plus the package-only LMR adjusters - `ReducedFutility` (with `ReducedFutilityRefDepth`), `MoveCountLmr`, `LmrChecks`, `NmpPackage` and the quiescence evasion prune removed from the engine; fourteen more settled as ties and stay off. A defect found while investigating: the training validation split leaks (5.48% of one shard's val tail found verbatim in 1.64% of the training corpus). See CHANGELOG.md and NNUE_HISTORY.md for the full account of why the data-step gain is small (the project's largest NNUE wins were learning-rate anneal tails, not data steps) and the experiment plan for the next generation. CI node count unchanged (net swaps never move the classical-path reference). 461 tests. | shipped |
| **5.9.16** | **SHIPPED 2026-09-22: a transposition cutoff teaches the move that caused it.** A node cut by the transposition table returned at once and its quiet move, the one that had failed high, learned nothing; only searched cutoffs fed the history tables. `TtCutoffHistory` ON in its third form: a quiet transposition move failing high at a non-PV node earns about 0.56x of a searched cutoff's depth-squared bonus in the butterfly and first continuation tables, capped at depth 6 as the reference caps `min(112 * depth, 695)`. **Fixed-node SPRT at 100,000 nodes against v5.9.14: +16.6 +/- 12.2, LLR +3.00, H1 over 1,300 games (LOS 99.6%)**; the full uncapped bonus had read +3.2 and the reference's malus -18.4 H0. Razoring removed again after a fifth, fully corrected shape (-13.0 over 409), the line closed. Node-identical otherwise (60 positions at depth 11, 4,902,968 nodes against v5.9.15 with the option on). CI node count regenerated. 461 tests. | shipped |
| **5.9.15** | **SHIPPED 2026-09-22: a checking quiet is no longer pruned blind at the leaves.** Futility pruning dropped quiets on the static eval alone and this engine has no gives-check test before the make, so a quiet check was pruned like any other move whenever the eval sat under alpha; the reference never futility-prunes a checking quiet. The first form (every direct check exempt to depth 4) measured -4.9 over 1,223 with +16.9% nodes. `CheckExemptFutility` ON in the bounded form, direct checks at depth <= 2 with SEE >= 0: **fixed-node SPRT at 100,000 nodes against v5.9.13, +13.0 +/- 10.2, LLR +2.97, H1 over 1,839 games (LOS 99.4%)**. Node-identical otherwise (60 positions at depth 11, 4,622,672 nodes against v5.9.14 with the option on): `LmrDeeperResearch` (H0 -4.4, measured on 2026-09-08 before the v5.9.11 `improving` fix and never re-measured alone), `HindsightReset` (-3.0 alone, -4.7 with the deeper re-search), `ReducedFutilityUnclamped` (-1.7) and `TtCutoffNodeType` (-2.0, then -6.6 on the corrected labels) removed with tombstones. CI node count regenerated. 461 tests. | shipped |
| **5.9.14** | **SHIPPED 2026-09-22: three slow positives that close together.** `TtNoPvCutoff` (no transposition cutoff at PV nodes, +6.4 over 2,920 alone), `FutilityFailSoft` (the futility-pruned quiets' value raises a fail-low node's bound, reference step 14, +8.3 over 2,293 alone) and `ImprovingAboveBeta` (improving |= eval >= beta after the null move, +5.8 over 1,206 alone) ON: **together +11.8 +/- 9.4, LLR +3.00, H1 over 2,259 fixed-node games against v5.9.12**. The KQK test `ABareKingTakesTheLongestRoadToMate` forbade Ka8 as the fastest loss; an independent brute-force mate solver says Kc8, Ka8 and Ka7 are all mated in 4 and only Kc7 in 3, so it now forbids Kc7. Node-identical otherwise: `CaptureFutility` removed after two forms (flat -2.0, flat -2.9), `PruneNpmGuard` removed (about -5 inside a bundle), `ReducedFutilityUnclamped` added for measurement. 461 tests. | shipped |
| **5.9.13** | **SHIPPED 2026-09-22: the null move reduces more the further the eval sits above beta.** `NmpEvalR` ON: the null-move reduction `3 + depth / 4` gains `min((eval - beta) / 81, 3)` plies (the reference's `(eval - beta) / 168` in its units, x0.48; capped at three because verification starts at depth 14), on the TT-refined eval, reading no cutNode. **Fixed-node SPRT at 100,000 nodes against v5.9.12: +14.4 +/- 11.1, LLR +2.96, H1 over 1,492 games (LOS 99.5%)**; bench -1.0% nodes. From the re-investigation of the discarded null-move options: `NmpGateDeepR` had licensed `R = 7 + depth / 3` only on a set reverse futility had already cut. Node-identical otherwise: `LmpCountsPruned` H0 (-17.4 over 680) and removed, kept as `HistoryPruneCounts`; `CorrectionGravity` (H0 -9.6) and `CorrectionWeightCap` (flat -1.2) removed, the correction family closed. Publish script fixed (it checked for the embedded network before `isready`). 461 tests. | shipped |
...
| **5.9.8** | **SHIPPED 2026-09-16: the shipping architecture's output layer was still scalar, and two corrections.** `FinishOutput` - the last stage of every arch-1/2/3 evaluation - was a thirty-two iteration scalar loop with a thirty-two deep chain of `long` adds; the identical construct for architecture 5, which nothing ships, was vectorised two versions ago and sits a hundred lines above it. **+3.87%**, plus **+1.29%** from two bounds-checked idioms in the same kernel (the last `new Vector<short>(array, index)` + `CopyTo(Span)` in the engine, and a four-`GetElement` fold). **CORRECTION - THE SESSION TOTAL IS MEASURED, NOT COMPOSED:** multiplying the five separate A/B ratios gives +14.6% and that is WRONG, because composing ratios taken at different times against different bases accumulates their bias and ignores overlap. End to end against the v5.9.5 baseline, three interleaved pairs, 450 paired positions: **+8.01%** [7.14%, 7.68%], 443 of 450 faster, about 7 Elo. **BYTE-IDENTICAL to v5.9.5** across all five changes: 29,268,779 nodes both sides, nothing differing. **CORRECTION - THE v5.9.6 CONTEMPT GRADIENT WAS NOISE:** a second experiment concentrated on the band that matters (Nalwald 3283 against our 3321) ran 145 games on that pairing and reversed it; pooled over 217 games contempt 25 scores 38.9% against contempt 0's 39.9%. What reproduces is that contempt lowers the draw rate in both experiments and gains nothing, because those draws are genuinely drawn positions. **Contempt stays at zero and the recommendation is not to enable it.** 456 tests. | shipped |
| **5.9.7** | **SHIPPED 2026-09-16: SEE stops rebuilding the attacker set it already had.** The swap loop called `AttackersTo` every iteration and that rebuilt the WHOLE set - twelve piece-bitboard reads, four attack lookups, two magic lookups - when only the slider half can change with the virtual occupancy. The fixed half is hoisted into five plain locals. **+4.14%** [3.46%, 4.48%], faster on 252 of 292 positions, p < 0.00001, **BYTE-IDENTICAL** over 29,268,779 nodes. Worth recording alongside it: the SAME idea measured **1.64% SLOWER** four hours earlier when implemented with a twelve-entry `stackalloc` cache, because that charges fixed setup to every call and most SEE calls end after one or two iterations. The idea was right, the implementation was what lost, and only the timed A/B could tell them apart. The three wins of the night multiply to **+8.9% search speed**, about 8 Elo at 65 Elo per doubling, with no change to any decision the search makes. 456 tests. | shipped |
| **5.9.6** | **SHIPPED 2026-09-16: the first search-wide profile, and the two free speed wins it found.** Every earlier profile covered the NNUE evaluation only. A CPU-sampling profile of the WHOLE search (`audit/profile_search.py`, cached 150 positions, depth 14, quiet box) puts self time at: NNUE 43.1%, search control 28.9%, ordering 12.1%, SEE 7.0%, movegen 3.9%, make/unmake 1.2%, TT 0.08%. Recorded so nobody chases them again: the accumulator is already lazy (34.7% of eager updates applied, finny table at 99.9%), the L1 dot already folds four output rows per pass, and R2R costs no speed against JIT (identical nodes, 44.1 s vs 44.3 s). TWO WINS, both BYTE-IDENTICAL over 29,268,779 nodes and both measured with paired interleaved runs plus a sign test: **TT prefetch +2.62%** [1.84%, 3.26%], 204/293 positions faster; **the coarse lane skipping its dead rows +1.93%** [1.29%, 2.50%], 197/292 faster - 62 of the 144 relation buckets have their row AND colour mirror entirely zero in the shipping net, so the lane was running two full passes over 128 int16 lanes adding nothing. Together **+4.6%**, about 4 Elo at 65 Elo per doubling, with no change to any decision the search makes. **`Contempt` ships INERT at zero and follows the rating gap**: the engine had no contempt at all and never parsed UCI_Opponent, which sits directly under the 80%-draw rate against opponents within 50 points. Self-play cannot measure it (both arms carry it and it cancels), so a round-robin against an outside field decided, and it says a FLAT contempt is the wrong shape: +16.7 points against the weakest engine of the field, +4.2 against the middle, -11.4 against the strongest - monotone in opponent strength, which is the order theory predicts. UCI_Opponent is now parsed and contempt scales with the gap: full at +100, zero at level or worse, never negative. An SEE rewrite was byte-identical but **1.64% SLOWER** and rejected - fixed setup cost on a loop that usually does not spin. 456 tests. | shipped |
| **5.9.5** | **SHIPPED 2026-09-15: the clock-lead sustainability guard now scales with a real lead.** A real rated game (25+5, 2026-09-15) showed the bot holding 10:27 against an opponent down to 0:29, a 21.6x lead, and still spending only about 11 seconds a move. `ClockLead` reads the opponent's clock and widens the target (capped at 2x), but the sustainability guard underneath (`inc + clock/16` target, `inc + clock/4` deadline) only ever read the engine's own clock and clawed back nearly all of the widened budget. The guard now scales by the same ratio when the lead is real; a clock deficit is untouched on purpose. Separately, `UseSlowTcEasyMoveDamp` (shipped ON in v5.9.3) had no `= true` initializer, the only "ships ON" flag in that file missing one - a host that never resends the option ran with it permanently off. 2 new tests anchored on the real game's numbers, 445 tests total. An SPRT against v5.9.4 (60+1, ponder) was cut short by explicit decision: 111 games, +6.3 +/- 34.0 Elo, LLR 0.04, no sign of harm - shipped on the code fix and its regression tests, not a concluded SPRT. | shipped |
| **field** | **RE-MEASURED 2026-09-09: the twenty labels the gauntlets solve against were measured directly.** A round-robin of the whole field at 60+0.6, 1,680 games, 160 per engine, no time losses. The labels claimed a 463 Elo spread; the games measure 668, with the two worst wrong in opposite directions by 151 and 136. Every gauntlet re-solved: 5.3.0 3284, 5.4.0 3303, 5.5.0 3296, 5.6.0 3315, 5.7.0 3286, 5.8.0 3296, 5.8.2 3321, 5.8.5 3319, 5.8.6 3296. The shift is uniform at 32 to 34 points, so the field was mis-scaled and not mis-ordered and no promotion decision changes. The measured rating now lives in each executable's filename and in every tournament script, verified across all 100 of them. |
| **5.9.4** | **SHIPPED 2026-09-14: the helper watchdog could crash the process; the repetition rule turns strict only when behind.** The v5.9.3 watchdog quarantines a helper that ignores cancellation for 3 s; when it finally returned it signalled the countdown of a LATER search, and a signal below zero throws on the worker thread and terminates the process (`InvalidOperationException` in `WorkerLoop`, recorded by the .NET runtime during the second gauntlet below). A quarantined slot never signals again; quarantine mark and signal are ordered by one lock. `RepetitionStrictWhenWorse` ships ON: the reference rule (a single repetition counts only inside the tree; the history needs a genuine threefold) while the root's static evaluation is -1.00 or worse, the current any-repetition rule otherwise. Measured the right way, against other engines (60+1, four threads, ponder, Winter 1.0, Pedone 1.5, Defenchess 2.2, Nalwald 19): the reference rule alone LOSES 73.4% to 78.1% with more draws (-44 Elo, 256 games) - any-repetition-is-a-draw is an anti-repetition drive when ahead; the hybrid ties, 73.4% to 75.0% +/- 55 (192 games), and removes the 0.00 claims at depth 23-41 reproduced in seven losing positions worth -1.8 to -10.0. The draws themselves, measured over 188 games with an independent arbiter: 80% draws against opponents within 50 points or below, all level at the moment of repeating; the engine does not outplay them, which is the net's job, not the search's. Gauntlet for the CCRL number pending. 443 tests. | shipped |
| **5.9.3** | **SHIPPED 2026-09-13: a 74-minute freeze traced and closed, two clock-management options ship.** A Lazy SMP helper stuck in the Syzygy decompressor's symbol-tree walk, the only unbounded loop of its four, with `_done.Wait()` blocking the whole engine on it. Watchdog (3 s) with quarantine, decompressor bounded at every data-driven step, dead `Syzygy.Hits` counter removed; node-identical 7/7 including two positions with millions of tablebase hits. `ClockDeficitBrake` ON: +14 against -44 Elo without it on a CCRL round-robin field (Nalwald, Iris, Rice, 180+2), cut at 126 of 200 games by a clear gap. `SlowTcEasyMoveDamp` ON as a judged tie of bounded risk (19 better, 20 worse, 10 equal over 49 judged position pairs at real clock context), backed by the measured finding that the engine spends a falling share of its nominal budget as the control slows (0.567, 0.460, 0.426, 0.380 bullet to classical; 0.427 to 0.173 in the back half of the game). Gauntlet for the CCRL number pending. 443 tests. | shipped |
| **5.9.2** | **SHIPPED 2026-09-11: the complete fqcohuman, sixty epochs.** Gauntlet, single-threaded, against twelve engines of field 2 whose labels were MEASURED on 2026-09-09: **52.1% over 240 games, 3321 CCRL +/- 45**, against v5.9.0/v5.9.1's 3276, v5.8.7's 3286 and v5.8.6's 3296 on the same measured labels. Measured with the box loaded: the corpus generation ran on twenty-five threads throughout, and it is never paused. The seven missing epochs trained under the new loader guards without a single skipped chunk or batch; the best checkpoint (epoch 6 of the stage, validation 0.005804 vs 0.005860) beats the epoch-14 net it replaces: **+21.6 +/- 14.6 Elo, LOS 99.8%, LLR 2.98, H1 over 983 fixed-node games**. `PonderContinue` ships as an option, off, pending a ponder-on measurement on a quiet box. Bench 7,793,209; 443 tests. | shipped |
| **5.9.1** | **SHIPPED 2026-09-10: a search that crashes answers with the best move it found.** Gauntlet, single-threaded, against twelve engines of field 2 whose labels were MEASURED on 2026-09-09: **46.2% over 240 games, 3276 CCRL +/- 44**, against v5.8.7's 3286 and v5.8.6's 3296 on the same measured labels. The number belongs to v5.9.0 and v5.9.1 alike: the search is identical to the node (bench 8,193,088) and the 5.9.1 change only decides what is answered when an exception interrupts a search, which never happens in a gauntlet. Measured with the box loaded: the corpus generation ran on twenty-five threads throughout, and it is never paused. Found live by the user watching a rated game: depth 36, score 0.00, principal variation starting Qb6 (a correct drawing repetition), then a `FileNotFoundException` inside the search and `bestmove c5c4` - the first move in generation order, hanging the queen, and a bishop three moves later. The handler exists so a GUI never sees the engine hang, which is right; answering with an arbitrary move when the search has already reported one is not. It now sends the best move reported so far, checked legal. The missing assembly was ours: the bot's self-contained binary was replaced IN PLACE while the game was running. Rule recorded: stop the bot before replacing its binary. Bench 8,193,088, identical to 5.9.0 to the node; 443 tests. |
| **5.9.0** | **SHIPPED 2026-09-10: the coarse-lane human net.** fqcohuman measured **+11.5 +/- 9.2 Elo, LOS 99.3%, LLR 2.97, H1 over 2,627 fixed-node games** against the net shipping since v5.4.0. Same binary both sides, only the model differing, so the gain costs no speed. More training on known ground rather than a new idea, and the size says so: the big network gains here came from changing what the data or the representation is (factorization +195, corpus scale +182, deeper labels +117), not from more epochs. It survived two crashes - a reboot at epoch 39 of 60, answered with `--init-from`, and a real loader defect at epoch 17 of 21 that sized a shuffle from a counter instead of the array. The shipped checkpoint is epoch 14 of the continuation, 53 of 60 planned, validation loss 0.005860, the best of the series; the last seven epochs are training and will be measured against it. Bench 8,193,088; 443 tests. |
| **5.8.7** | **SHIPPED 2026-09-10: two guards on the tablebase win band, both NEUTRAL.** Gauntlet on field 2, single-threaded, against the twenty engines whose labels were MEASURED on 2026-09-09: **47.5% over 240 games, 3286 CCRL +/- 44**, against v5.8.6's 3296, v5.8.5's 3319 and v5.8.2's 3321 on the same measured labels. Ten points below the previous release and well inside the interval of either number, which is what a neutral change should look like. Measured with the box loaded: the 5.8.6 corpus generation was running on twenty threads throughout, and it is never paused. `WonBandMaxMen` (8) keeps the won-band tie-break near the tables it was designed for; it was firing with a median of ten men and up to twenty-one. `WonBandPromoGuard` (on) counts what the opponent can promote alongside what it can capture. An independent judge over the 29 positions where the band fires: one and two moves changed, median difference zero. Bench 8,478,755, IDENTICAL to 5.8.6 to the node. They ship on the user's call at zero measured cost. The investigation did not find its defect: a reference engine 400 points stronger scored 0.520 on the same fifty positions against this engine's 0.495 over 100 games each, so the positions are hard rather than the engine weak. Four measurement errors on the way, all recorded. 443 tests. |
| **5.8.6** | **SHIPPED 2026-09-08: the search corrects a reduction it now regrets.** Gauntlet on field 2, single-threaded, the same field as v5.3.0 to v5.8.5: **48.8% over 240 games, 3328 CCRL +/- 44** (v5.8.5 3352, v5.8.2 3353, v5.8.0 3328), measured while a 21-engine round-robin shared the box. The 24 points against v5.8.5 sit inside the +/- 44 of either number, so this does not contradict the +17.0 H1 the same change measured at fixed nodes; it says the gauntlet cannot resolve a difference that size. The field's labels were measured on 2026-09-09 in that round-robin and this number became **3296 CCRL** on the measured labels; the whole series moved down 32 to 34 points, uniformly, so the ordering is unchanged. `HindsightDepth` ON (a node revisits its parent's late move reduction with its own static evaluation: one ply deeper when the reduction was undeserved, one shallower off the PV when it is confirmed): fixed-node SPRT +17.0 +/- 12.5, LLR +2.98, H1 over 1,266 games. From the sixth audit pass, which also produced `PriorFailLowBonus` (-22.2, H0), `LmrDeeperResearch`, `RfpTtMoveGuard`, `LmpCountAllMoves`, `DrawRandom` and `CutoffCountLmr`. Bench 8,478,755; 441 tests. |
| **5.8.5** | **SHIPPED 2026-09-08: quiescence tries the checks.** Gauntlet on field 2, single-threaded, the same field as v5.3.0 to v5.8.2: **51.9% over 240 games, 3352 CCRL +/- 45** (v5.8.2 3353, v5.6.0 3347, v5.8.0 3328), measured with the box also running four fixed-node SPRT chains. The field's labels were measured on 2026-09-09 in that round-robin and this number became **3319 CCRL** on the measured labels; the whole series moved down 32 to 34 points, uniformly, so the ordering is unchanged. `QsChecks` ON (checking quiets at the first quiescence ply): fixed-node SPRT +6.3 +/- 6.5, LLR +1.20 (LOS 97.2%, stopped and promoted by the user's call, the [0, 10] bounds would not close either way), H1 over 4,993 games, the slowest verdict of the campaign. Bench 9,084,675; 441 tests. |
| **5.8.4** | **SHIPPED 2026-09-08: the tablebase-won root pays scout prices.** `TbWinTieBreak` with a band-edge scout: a null window on the band boundary proves a root move band-won, a second probe above the band finds mates, and only those get the exact search. Judge on 40 won positions: off 127.9, on 93.7 (300 or more: 20.0% to 12.5%), the same conduct as the full-window version (167.3 to 83.8). Cost check with tablebases: 0.0 +/- 9.9 over 2,074, LLR -1.94, stopped at a measured zero (455 wins to 455) by the user's call (the full-window version had read -7.5 +/- 12.7 over 1,293). Bench 8,961,530; 441 tests. |
| **5.8.3** | **SHIPPED 2026-09-08: the search is deterministic.** The v5.8.1 quiescence repair ships by the tie criterion: +1.0 +/- 7.1, LLR -3.0, H0 over 4,109 fixed-node games (the stack write alone -1.2 +/- 8.9 over 2,673); three searches of six bench positions identical to the node, pinned by `SearchDeterminismTests`. Regression test for the tablebase-won tie-break; UCI band formatting covers aspiration bounds. Measured and left off: `NmpNonPvOnly` (+4 to +7 over 3,000 games, never closed), `CaptureSeePruneDeep` (-0.1 +/- 9.6 at 2,408). Bench 8,961,530; 441 tests. |
| **5.8.2** | **SHIPPED 2026-09-08: quiet moves that lose material are pruned.** Gauntlet on field 2, single-threaded, the same field as v5.3.0 to v5.8.0: **52.1% over 240 games, 3353 CCRL +/- 45**, the highest of the series (v5.6.0 3347, v5.4.0 3337, v5.5.0 and v5.8.0 3328), measured on the same loaded box as the v5.8.0 number (four fixed-node chains and a four-thread match running alongside). **Package control (2026-09-08):** the shipped v5.8.2 against the shipped v5.8.0 at 100,000 nodes, defaults against defaults: **+28.8 Elo +/- 17.6, LLR +2.96, H1 over 652 games** (170 wins, 116 losses, 366 draws). The three prunings switched on since v5.8.0 were each measured alone; together they hold, inside the interval of their sum. `QuietSeePrune` ON (depth 8 and below, SEE loss beyond `23 x depth^2`): fixed-node SPRT +15.7 +/- 11.8, LLR +2.96, H1 over 1,657 games. Bench 8,071,333; 438 tests. Still running: `NmpNonPvOnly` (+16.4 +/- 17.9 at 711), `DetVsOld`, `QsStackMove`, the rest of the chains, the ClockLead round-robin at 180+2 and the 5.8.0 gauntlet. |
| **5.8.1** | **SHIPPED 2026-09-07 (night), rebuilt the same night with `TbWinTieBreak`: a tablebase win no longer bought with pieces** (seven or more men: the band score `TbWin - ply` rewarded reaching the tables sooner, and a hung piece is the fastest way there; full-window root in the won band keyed by material kept plus progress; the game position queens at every depth; gate by an independent engine mean loss 167.3 to 83.8 centipawns, moves costing 300 or more from 25.0% to 12.5%, 150 or more from 32.5% to 17.5% (40 positions, one thread, depth 14, bench node-identical); the clock-lead scale now obeys the sustainability guard). **Late move pruning at every depth, the stored score in the pruning, and a clock lead spent.** `LmpAllDepths` ON: fixed-node SPRT +16.3 [+4.2, +28.4], LLR +2.98, H1 over 1,515 games. `TtEvalRefine` ON: +13.1 +/- 11.6 at 1,727 games, LLR +2.32, interval clear of zero. `ClockLead` ON at the user's request: optimum x (our clock / their clock), capped 2x, only when ahead; 18.9 s to 37.8 s and depth 22 to 23 on the 8:08-vs-2:26 rapid position; confirmation against outside engines at 180+2 with ponder running. **ClockLead against outside engines (2026-09-08):** round-robin at 180+2 with ponder, four threads and tablebases, the bot's own regime, against Iris 2.0, Rice 8.0.0 and Nalwald 19, 48 games per engine: **`ClockLead` on 63.5% (+97 Elo +/- 69, first in the table), the same binary with it off 43.8% (-44 +/- 60)**, no loss on time in 120 games. One disconnect per arm (games 48 and 49) came from an operator kill of unrelated processes that shared the binary; symmetric, and the comparison stands. Found: the search was never deterministic across searches (quiescence correction keyed on stale stack slots, all releases); the real keys measure -9.9 +/- 17.1 at 718, so the measured quiescence ships and the repair (`QsStackMove` with `QsContCorrection` off or `QsEntryKey`) is a pinned configuration under measurement. Debug-only `HasFlag` boxing removed. `NOA_SEARCH_STATS` instrumentation (first-move cutoff 86.7%, TT move at 19% of loop nodes). Options queued: `Razoring`, `QuietSeePrune` (running, +22.6 at 206), `CaptureSeePruneDeep`, `QsChecks`, `ProbCutAllowNull`, `FutilityFailSoft`, `CaptureFutility`, `SingularTight`, `TimeScale`, picker magnitudes. Bench 8,134,147; 438 tests. |
| **5.8.0** | **SHIPPED 2026-09-07: the transposition table was throwing away its best moves.** A fail-low node stored the best of its failing moves as the position's transposition move and overwrote whatever an earlier visit had proved; the reference stores no move on a fail-low. `TtKeepMoveOnFailLow` ON: bench 14,994,140 to 9,658,211 nodes at depth 12, fixed-node SPRT +43.3 [+20.9, +65.7], LLR +3.0, H1 over 411 games. From a second line-by-line read of the whole engine against the reference and three other open engines. Also exact: four decisive-score guards that stopped at the mate bound and let the tablebase band through (null move, ProbCut twice, singular). Nine divergences as options, off, with node counts and fixed-node SPRTs queued: RepetitionAfterRoot, NmpNonPvOnly, TtEvalRefine, TtMateReuse, Razoring, LmpAllDepths, QuietSeePrune, CaptureSeePruneDeep, RootScoreOrdering; TimeScale queued at 60+1 after the bot's clock measured at 84% of its opponents' spend over 483 games. 434 tests. |
| **5.7.0** | **SHIPPED 2026-09-07: a lost position stops being an excuse to give pieces away.** The evaluation saturates once a position is decided - at -7 pawns, a knight is worth almost nothing more - so the material gradient vanishes exactly where it is needed. In a bot game at 180+2 the engine scored a knight sacrifice at -698 against -707 for the sane move and played it: a knight for nine centipawns. REBUILT THE SAME EVENING: `LostResistance` measured WORSE by an independent 3461 judge at every threshold (99.8 without, 141.0 at -600) because its gate was circular; OFF. Replaced by two tablebase repairs from a reference-judged blunder map (0.8% blunders near equality, 16-26% below -750): a lost root keeps the in-search probe on regardless of the fifty-move counter, and pawnless lost roots keep the longest defences by DTZ. Final gate over 101 lost positions: mean loss 121.3 to 91.6, whole-piece blunders 14.9% to 11.9%, bench node-identical. The original text follows for the record. `LostResistance` shipped ON at noon: below -600 and above the tablebase band the root searches every move full-window, rounds scores to whole pawns and breaks ties by what the opponent can capture for free. Bench node-identical (14,994,140), so nothing above -600 changes. Over 25 positions from the bot's own games material given away fell 400 to 200, one better and none worse. Clock cost not yet measured; its drawn-position sibling DrawTieBreak measured -29.5 H0 the same day and stays off. |
| **5.6.0** | **SHIPPED 2026-09-06: the audit release - the hot path read line by line, and no SPRT gates it because nothing that touches play can change a result.** Gauntlet on field 2, single-threaded, measured on the rebuilt binary: 51.2% over 240 games, 3347 CCRL +/- 44, against 5.5.0's 3328 +/- 44 and 5.4.0's 3337 +/- 41 on the same field - the highest of the three, bands overlapping. The run on the pre-fix binary was stopped at 105 games (49.5%, 3325 +/- 67) to free the machine and is kept. The release was rebuilt on 2026-09-07 for a defect three bot games exposed: inside the tablebase-lost band material is free, because the score records only how many plies separate the line from the tablebase, and the engine gave a rook away. TbResistance, on by default and built for exactly this, could never fire: its key gave the band's own score 4096-to-1 priority so it waited for an exact tie the band never produces, and it ranked by a static evaluation blind to the capture the move walks into. Band flattened inside the key and the key changed to material: over 23 positions from the bot's games, one thread at fixed depth, material handed over fell 2,440 to 420 and whole-piece giveaways 4 to 1, seven positions better and none worse, bench node-identical. Mate distance pruning, missing since the engine was born, ships ON: both clamps are theorems, the 60-position bench returns 14,994,140 nodes with it on or off to the node, and a mating suite finds the same move with the same mate distance for 239 nodes instead of 308,528 (mate in 1), 2,319 instead of 748,527 (mate in 2), 16,282 instead of 183,934 (mate in 3) - clock the engine used to spend confirming a mate it had already found. The `bestmove 0000` an illegal root could produce was an `IndexOutOfRangeException` present in EVERY release: the search captures the enemy king, that side then has no king, and the attack lookup reads past its table; unreachable from any legal position, confirmed absent from every stored log, guarded at the root anyway. Five node-identical speedups (quiescence legal-move probe, dead continuation-history reads, SkipLocalsInit, tablebase list pool, and the board history moved to arrays with packed Zobrist keys) measure +4.3% [+3.1%, +5.2%] on a QUIET machine, paired over ten alternating runs and 590 positions with a same-binary control at -0.3% (p = 0.52); the +6.7% to +9.4% first taken with an SPRT and a training sharing the machine was too high, because contention inflates what pairing can cancel. About 4 Elo at 65 per doubling. Four findings ship as measured-off options with their node costs: RootStaticEval +2.0%, QsStackMove +10.8%, CheckExemptFutility +16.9%, TtNoPvCutoff +0.16%. Closed by reading: the ProbCut gate is faithful to the current reference, and the quiet partial sort has no exact lever. 425 tests, including zero bytes allocated per node, the repetition scans against a naive re-implementation, every legal root returning a legal move, and the network scoring a position and its mirror identically. Fourteen awkward UCI sequences answer once each with one thread and with four. |
| **5.5.0** | **SHIPPED 2026-09-06: the clock rules stopped treating a lost position as a decided one.** The easy-move cut tested the absolute score, so 42% of the moves played below -7 were instantaneous (754 bot games, 37% of them reach that band). Now winning-only, and suspended under fifty-move pressure. Self-play -4.5 [-21.0, +12.1] over 390 games with zero measured clock cost; bot behaviour 41.9% to 9.0% instantaneous. Ships under the tie rule. Gauntlet field 2: 3328 CCRL +/- 44 (48.8% over 240), level with 5.4.0. |
| **5.4.0** | **SHIPPED 2026-09-02: fqhuman +18.7 [+5.3, +32.0] SPRT H1 over fq594 in 1,360 fixed-node games - the champion recipe on the corpus extended ~594M -> ~924M with the HUMAN game segments finally added (the provenance-bug debt: the human datagen had silently never run), fourth consecutive net promotion by one protocol (fqwd0 +11.1, fqmix +19.6, fq594 +29.7, fqhuman +18.7). Gauntlet on field 2 (same field as 5.3.0, so comparable): 50.4% over 277, 3337 +-41 CCRL - +20 over 5.3.0's 3317 +-44, in line with the fixed-node +18.7. Interrupted by a host restart at 37 games and resumed on the same pgn.** The release also carries: the exporter's accumulator headroom guard made SCHEMA-EXACT after rejecting this very net on a bound that mixed king buckets that never share an accumulator and squares that cannot hold two pieces (32,770 "failure" vs a real worst lane of 12,948 over 4M positions; the previous champion re-exports byte-identically under the new guard, and the int16 limit itself was never touched); the transposition table rewritten for multi-thread coherence (64-byte aligned pinned clusters so a probe is one cache line, and the probe's generation refresh gated to actual changes so a hit stops invalidating every other worker's cache; node-identical, single-thread neutral at +0.8% pooled with pairs disagreeing in sign); TbResistance ON (a resistance gradient at the root of a tablebase-lost position; TbPvCap failed its behavioural gate and stays inert - and TbResistance itself could not act until v5.6.0 repaired its selection key, see that entry); the ponder freeze hole closed unconditionally with the in-place conversion tombstoned (H0, three candidates - the relaunch inherits the TT's confidence); and THE RYZENGATE CASE CLOSED: the 24-thread collapse (1.05x of one thread, and a Threadripper matching a 2018 Mac Mini in production) was machine OVERSUBSCRIPTION - an uncapped training holding ~15 threads under every measurement - not an engine defect. The clean-machine control (never run before) scales 2.13x/2.14x/1.97x at 8/16/24 threads; the three repairs all tied at deployment thread counts (depth skip +1.8 in 195, widened vote franchise +2.4 in 438, aspiration pin unlaunched on the same prior) and stay as inert options with their numbers; the strong open engines were read line by line on the way (none skip depths; the whole per-thread diversification is delta += threadIdx % 8; the vote counts every thread gated by score provenance, not depth; the table probe is a pure read); and the real fix ships as an operational rule - deployment-thread engines never share a machine with an uncapped training (capped at 4 low-priority threads the same machine holds 2x). Banked along the way: a validated fast SMP bench recipe (20+0.3, concurrency 1, no tablebases, GC heaps capped - 10+0.1 produced 60% time forfeits on BOTH arms and was discarded), and the psqt-on-champion retry woken (its written condition, "if fqmix wins", was met twice). 378 tests. Both bots on 5.4.0, hash-verified both platforms, Mac playing from 21:11 CEST on release day; rollbacks in the engines archive | SPRT H1 passed at release; gauntlet on field 2 immediately after with the coarse training frozen for the duration (recorded, not hidden). In flight at close: fqcoarse (the coarse-threat lane on the champion recipe, the surviving route to the +60.4 threat bank) training on the GPU; the 24-vs-4-thread prize match and the per-core Mac ratio queued for the first clean-machine slot |
| **5.3.0** | **SHIPPED 2026-08-30: fq594 +29.7 [+12.2, +47.3] SPRT H1 over fqmix in 763 fixed-node games - the volume axis measured alone for the first time (594M vs 324M, recipe verbatim, same teacher and label depth) and it pays the second-largest net gain after factorization. Gauntlet on the NEW field 2 (mean 3337, four bridge anchors shared with field 1): 47.3% over 240, 3317 +-44 CCRL - a SERIES BREAK, not comparable 1:1 with field 1's 3342 +-86; bridge-anchor performances 3334-3451 with Patricia 3281 at 57.5% against 55% before, and a monotone gradient from 85% at 3150 to 12.5% at 3450.** The search half: **statScore in LMR ships ON (+20.9 [+7.2, +34.7] H1 over 1,099)**, the one survivor of the resurrection campaign; its seven companions all measured H0 on the statScore-on base and stay as inert tombstoned options - cutNode -10.7 (third and final burial, now with IIR, statScore and the ttMove sub-term present), fail-low correction learning -11.7, the moveCount pair -7.4, the dynamic initial aspiration window -8.6 (the classical "fixed window won" verdict survives re-measurement), the reference history-bonus shape with the consumer rescaled to constant mean torque -30.7 in only 510 (the picker's constants are tuned around depth-squared ranges and a rail-saturated butterfly swamps them), correction magnitude in LMR -15.1, and removing the killer/counter shallowing -15.5 (not redundant with statScore; it stays). The campaign rule those numbers wrote: on this engine's softer reduction curve the reference's standalone LMR terms do not transfer - the one thing that paid was a NEW SIGNAL at consumer scale. Also: the histstats instrument rebuilt as a UCI command (consumer-site sampling; the July one was lost with its branch); the statScore capture branch found structurally dead (LMR sits behind the quiet gate; capture LMR is a future axis); the CI relanded fixed from closed PR #70 with the nodecount reference regenerated at 258,577 on the classical path. 378 tests. Both bots on 5.3.0, hash-verified, rollbacks kept | Both halves of the bar passed: SPRT H1 and the gauntlet anchor. Since the close (2026-08-31, toward 5.4.0): the Bothev2 case diagnosed and fixed - in tablebase-LOST positions the flat -TbWin+ply score rewards delaying TB ENTRY rather than the defeat, so the engine let pawns promote instead of capturing them (108 of 367 bot games in four days sat in that band); the fix searches lost roots full-window and breaks ties by the child's real evaluation, behavioral gate passed on the game position, measured a tight zero with tablebases on both arms, and ships ON. The coarse-threat pipeline (gate 2b) closed end to end with exact integer parity - encoder, CSR companions, trainer lane, export block, engine loader and an evaluation-time lane - awaiting only the GPU for the real net. Capture LMR (the whole reduction block sat behind the quiet gate; the reference reduces captures too, and the capture half of statScore was found structurally dead) is in SPRT. The ReadyToRun A/B measured no credible steady-state difference (sign test 89/171), so R2R stays for its cold-start win; NativeAOT waits for a truly quiet machine |
| **5.2.0** | **SHIPPED 2026-08-29: fqmix +19.6 [+6.4, +32.9] SPRT H1 over fqwd0 in 1,295 fixed-node games - the three loss-campaign axes STACK - and the gauntlet anchors 3342 +-86 CCRL (81.7% over 240, field saturating: 55% against its strongest member at 3281). The psqt two-headed net measured H0 alongside (-6.4 [-18.3, +5.5] over 1,691 games, lane verified exact); fq594 (same recipe, ~594M corpus) is in flight.** The loss axis that opened this version: fqloss2 +11.9 [+2.6, +21.2], LLR +3.08, H1 over 2,832 fixed-node games against fq60** - the reference-style loss (symmetric win probability, separate scales for net and label, exponent 2.5) with its constants REFIT to this corpus by logistic regression (offset 240, scale 145), against the raw MSE every published net had trained with. The raw reference constants had measured -41.8 H0, so the scale was the poison, not the loss. With fqwd0's +11.1 already shipping in 5.1.0, the stacked recipe (`fqmix`: no ft weight decay + refit loss + lambda schedule 1.0 to 0.7, the last at +5.3 unconcluded on its own) trained, met fqwd0, and the axes added: +19.6 H1. Also measured between releases: the reference's all-depth shallow pruning ladder (all-depth LMP, capture-SEE margin fed by capture history, continuation-history pruning, parent futility on lmrDepth, quiet-SEE on the reduced depth), both variants H0 - **-8.8 over 1,303 games complete, -13.7 over 938 with the futility rung removed** - kept in the tree as inert off-by-default options with a named precondition for any retry: port the full reference reduction curve first, because the ladder's constants assume reductions this engine does not make. The psqt two-headed net (engine lane, loader, finny cache, trainer head and parity tests all landed with 5.0.2.1) is training now, and the corpus is being extended by 270M positions labelled at 6,000 nodes by the current engine's search with the teacher net held fixed - volume stays the only moving axis | Both halves of the bar passed: SPRT H1 and the gauntlet. Bot deployment follows |
| **5.1.0** | **SHIPPED 2026-08-27: +11.1 [+2.3, +19.9] SPRT H1 over fq60 (fqwd0, no ft weight decay), gauntlet 3242 +-25 CCRL (72.9% over 240, statistically level with fq60's 3271 +-40; the SPRT carries the delta). Both bots deployed, hash-verified.** THREATS ARE CLOSED AND WIDTH IS OPEN, both by measurement. Threats win the evaluation outright (**+47.8 Elo [+21.6, +74.5] at fixed nodes, H1**) and lose the game (**-14.9 [-30.1, +0.1] at 180+2 over 723 games, H0**), so they do not ship. Width 512, reburied for weeks on a -30.3 that compared a brand cut at EPOCH 5 OF 60 against a converged one, wins **+38.4 [+18.7, +58.3] at fixed nodes over 637 games, H1** once trained to convergence; its clock SPRT concluded on 2026-08-25: **-27.0 [-48.0, -6.3] at 180+2**, stopped by hand at 97% of the way to H0 with the whole interval negative - the evaluation gain does not survive its speed. With width 256 at -31.9 (converged, at the clock) and clean output buckets at -49.7 (both arms arch 3, 60 epochs), **the capacity axis is now closed by measurement in every direction**. **The byproduct is worth more than either: crossing +47.8 at fixed nodes with -14.9 at the clock at 0.510x NPS calibrates, for the first time in this engine, that a doubling of speed is worth ~65 Elo at 180+2** - a constant previously borrowed and guessed anywhere between 0.58x and 0.70x. It reframes every capacity question as a question about CHEAP capacity, and it prices width 512 at 0.655x against a 0.664x break-even, i.e. exactly on the line. **CLOSED 2026-08-26, all by measurement:** the 512 clock SPRT stopped at **-27.0 [-48.0, -6.3]** (evaluation gain eaten whole by speed), width 256 **-31.9** converged, clean output buckets **-49.7** H0, EarlyLmr **-2.6** H0 over 1,717 games, WideWindow **-5.0** H0, and root optimism, the last unmeasured architectural gap from the reference audit, **-0.6 [-8.9, +7.7] H0 over 2,747 games** - a tight zero, kept in the tree as an off-by-default option. The capacity axis and the search-gap vein are exhausted; what remains is the NET: the reference-loss, lambda-schedule and ft-weight-decay training arms the crashed 11-08 queue never reached (training now), and the psqt two-headed net whose engine and trainer sides are fully built and parity-tested | Architecture 5 measured and REJECTED at -32.8 [-56.5, -9.3] over 575 games. Nothing else here is measured, and node counts stay byte-identical for every net without threat features, so no published engine moves | Every change below is either behind a pending SPRT or inert on existing nets. Node counts are byte-identical for every net without threat features, so no published engine moves | **Three buried search blocks rebuilt, MEASURED, and REVERTED - plus the threat feature path built end to end and made incremental.** The search half is a negative result and the branch no longer carries it: the package measured **-22.6 [-41.7, -3.7] H0**, 5E alone **-13.1 H0**, 5G multi-level **-40.9 H0**, and the bonus-shape fix ALONE **-5.8**, neutral - so the levels are what cost, not the shape. Reverting restores v5.0.1 node for node (257616 / 447929 / 472466 at depth 12), the three numbers written down BEFORE the work began. 5E is withdrawn with a named precondition rather than buried: multi-cut prunes 44% of eligible nodes and does not respond to the margin or the gate, which says the TT move is often not uniquely good - a move ordering fact - so it gets re-measured after the killers and counter reform, with the knobs kept on branch bisect-5x. What follows is the record of what was built and why it was worth building even though it lost. An audit of every negative this project had accepted found that **five of six do not survive four questions** - did the arms converge, was the configuration in the regime where the thing is known to work, was there a positive control, and what difference with the reference remains. 5E singular, 5G multi-level history and 5C statScore were all measured against an engine whose move ordering was corrupt (killers returned before history was read, fixed in v4.4.0 for +8.0) and none was re-run afterwards, although the plan already said they would only "have somewhere to bite" after that reform. **Settled since: 5E re-measured and H0 again (-29.0); 5G re-measured and H0 again (-40.9); and 5C statScore, re-measured faithfully on 2026-08-29 with both continuation distances, WON: +20.9 [+7.2, +34.7] H1 over 1,099 fixed-node games - on by default from 5.3.0. The one resurrection out of three, and the audit that predicted one of them would pay was right.** Reading the source showed the implementations were also wrong: **singular had one of its four outcomes** - the margin was an invented `2 * depth` against the reference's `(59 + 66 * ttPv) * depth / 63`, so the extension almost never fired, and multi-cut, the negative extensions and the double/triple tiers were absent entirely. **Continuation history had one level** against the reference's six written and five read. **statScore could not have been ported faithfully at all**, because its formula reads TWO continuation levels and this engine had no `contHist[1]` until those tables existed. **Four defects found auditing that same day's work before measuring it**, none caught by the compiler or by 361 tests: singular extended one ply too many (newDepth computed after the `depth++`); statScore priced every capture by its own attacker (`VictimIndex` read after `MakeMove`); a double integer division left the far continuation levels learning **nothing** at shallow depth; and the same product overflowed 32 bits at the ply cap, which would have taught the tables the opposite of what happened. Continuation history also moved to `short`, halving six tables from 14.2 MB to 7.1 MB at byte-identical node counts. **THREAT FEATURES, whole path, nothing measured in Elo.** The schema was settled against the source and the previous encoder was not a half-scale version of it but a differently shaped space: the factor of two is the COLOUR OF THE ATTACKED PIECE, and the `from < to` bit is not a dimension at all but a deduplicator for symmetric relations. 60,720 dimensions, verified by enumeration - 54,092 relations, 54,092 distinct indices, zero collisions, and the 6,628-index gap accounted for exactly. **Arch 4** appends a second transformer summed into the same accumulator, with the header untouched because the row count is a constant of the schema and is asserted rather than read. **The accumulator now updates threats INCREMENTALLY**, which was the reason a threat net could be measured and never shipped: a full refresh costs 3,000-3,700 ns per node against ~1,000 for an entire evaluation. It could not reuse the lazy machinery, because that replays a recorded (piece, from, to) without the board and a threat feature depends on the whole position, including every discovered relation where a slider's ray opened or closed - so the delta is taken across TWO calls, one on each side of the move, with each side using its own affected set because the pre-move board is gone by the second. The affected set is provably complete: a feature changes only if its attacker, its target or its ray meets a changed square, and all three land in attackersTo of one - plus the pawn one square behind, because a pawn stopped by a pawn ahead is the one threat relation that is NOT an attack. **The parity test took two attempts to be worth anything**: the first checked every node and passed a deliberate sabotage, since asking for an evaluation materialises the level and quietly guaranteed every parent was ready before its child was pushed. Checking only at the leaves reproduces the chain the search really builds. NPS is NOT measured yet - laziness was worth +3.1% and this gives some back - so the fixed-nodes SPRT remains the plan and speed is a separate question. **A probe with a positive control** (feature factorization, worth +128 Elo in the field, moves validation loss 14.88%) reports threats gaining **3.96% at width 128, 5.35% at 256 and 1.68% at 512** with six arms converged, while widening ALONE buys 0.11% - input and capacity are coupled, which reinterprets the -30.3 that closed the width axis. Parity checked at every joint: encoder against python-chess, C# indices against Python on 256 cases, model file against loader, and the engine's evaluation against the file's own arithmetic (79/79, 70/70, 91/91). Also here: the WPF title bar follows the Windows theme, which it never did because the DWM paints it light unless a window asks. 370 tests. |
...
| **2.8.3** | **+24.4 +/-17.5 vs 2.8.2 (835 games, interval excludes zero); LTC gauntlet +112 +/-24 relative** | **History gravity that actually acts.** v2.8.2's "bounded gravity" was **numerically inert**: `score*|bonus|/MaxScore` truncates to 0 with a 2^20 bound against values near 7,000. The butterfly bound was sized at 7183 like the reference: mean +71.8 -> +13.5, tail 6086 -> 3134. LMR pipeline moved to fixed point (1024ths), **verified neutral** by identical node counts. **Cut along the way:** statScore as the LMR history term, re-measured at the real time control, -18 Elo H0 - closing the evidence gap from 5C, which had judged it at 5+0.05. [WARNING] **The SPRT was stopped by hand at LLR 2.61 against a 2.94 bound: it is not a formal H1.** 276/276 tests |
| 2.8.2 | **SPRT H1 +28.0 +/-17.2 vs 2.8.1 (834 games); LTC gauntlet +94 +/-23, ~3013 +/-30 CCRL** | **Validated classical-search audit, without pulling NNUE/SMP forward.** Pawn correction history; ProbCut with depth>=1 verification and promotions exempt from the simplified SEE; fixed aspiration window with fail-low recentering; proven killer/counter bands kept; not unconditional check extension; explicit-only UCI logging. 276/276 tests. [WARNING] This row also credited "continuation history by gravity": **corrected on 2026-07-23, that gravity is numerically inert** (`6086*169/2^20` truncates to 0) and the +28.0 came from the rest of the package. |
...
| (5C) | [CUT] CUT 2026-07-18; **CLOSED 2026-07-23** | The reference LMR suite + statScore. At the real time control: bundle -9.7, rebuild -25.7, statScore machinery -10.8; statScore-in-LMR re-measured at 10+0.1 over the 1024ths pipeline **-18 Elo, 47.4%, H0**. What DID survive and ship: the fixed-point pipeline (v2.8.3) and the ttCapture/ttPv adjusters (v2.8.4). Permanent findings: 1024ths granularity was an unidentified prerequisite (now converted and verified neutral), and the butterfly table is skewed by construction (mean +71.8 vs median -8) |
| 2.7.1 | **+2.9 +/- 7.4 pooled SPRT over 4347 games; +44 +/- 23 rel LTC; ~2970 +/- 25 CCRL** | 5B scope-cut: NMP verification at depth=14 (nmpMinPly), fail-soft NMP, statScore term in the RFP margin + eval=beta guard, statScore stack x0.28 measured; WAC 262/300 with -21% nodes at d15 / -45% at d16. Plus mate fixes: ID no longer breaks on mate scores (a longer defense when losing), UCI `score mate N` |
| 2.7.0 | +4.0 +/-27.1 SPRT (stopped at 380 games, LLR ~0); **~2965 +/-25 CCRL measured** (624 games, **+43 +/-23 relative vs 2.6.9's +16 on an identical field/TC - the search gain GROWS at LTC**) | Improving flag (5A): static eval per ply, `eval[ply] > eval[ply-2]` modulates LMR (+1 ply when worsening), RFP (margin x(depth-improving)) and LMP (threshold halved when worsening) - Previous |
...
## [DONE] BLOCK 2 - Base classical evaluation (v2.4.0)
...
## [DONE] BLOCK 2.5 - Fine classical evaluation (v2.4.5)
...
## [DONE] BLOCK 3 - Speed / Movegen (v2.5.0)
...
## [DONE] BLOCK 4 - Reference-level classical evaluation (v2.6.x) - COMPLETE
...
### [DONE] 4A - attackedBy infrastructure (v2.6.0) [WARNING] MANDATORY PREREQ
...
### [DONE] 4B - Threats (v2.6.1)

**Status: DONE; Branch: `2.6.1`; SPRT: passed (+103 +/- 35 Elo vs 2.5.0, llr 2.99, H1 at 243 games, 64.4%)**

Far above the estimate (+25-35): the project's biggest evaluation jump. Critical lesson folded into the golden rules: reference values are ALWAYS rescaled x0.48 (the first attempt with raw values trended to llr -1.09).
...
### [DONE] 4C - Non-linear mobility (v2.6.2) - DONE

**Status: DONE (reference tables x0.48 re-centered, x-ray, reference mobility area, pin restriction); SPRT vs 2.6.1: +6.6 +/- 11.5 Elo, LOS 87%, 2000 games (bounds not reached; kept anyway: it is prerequisite infrastructure for 4D/4E)**
...
The current linear model (`MobilityStep * (moves - baseline)`) loses Elo because going from 2->3 squares for a knight is worth 5x more than going from 7->8. The reference uses a 32-entry lookup table per piece (MG+EG).
...
### [DONE] 4D - Shelter / Storm + full King Safety (v2.6.3)
...
- **EG king-pawn proximity** - shelter - 16 x minPawnDist in the endgame (the king must approach his pawns).
...
- `Weak squares in king zone` - 183 x popcount of weak squares in the king zone.
- `King flank attack / defense` - 3 terms: flank attack, flank attack^2, flank defense.
...
[WARNING]Benchmark NPS before/after. If it costs >5% NPS, find which components are the expensive ones and isolate them.

---

### [DONE] 4D.5 - Adaptive time management (v2.6.4) - DONE AND **SUPERSED BY v2.6.5**

**Status: CLOSED. This is not pending work and it is NOT to be redone.** It carried a - the only one in the document, against 16 [DONE] and 2 [CUT] - left over from when it was still in the air. Corrected on 2026-07-23.
...
### [DONE] 4E - Missing piece terms + reference timeman (v2.6.5) - DONE (REVISED)
...
3. **KnightOutpost keeps the texel value S(51.18)** (lowering it to the generic x0.48 lost measured Elo); `BishopOutpost` scaled by the same ratio -> S(29.13).
4. **The reference's complete timeman** (pulled forward from 5H, explicitly requested): `TimeManagement::init` verbatim (optimum/maximum, both TC shapes, the whole increment folded into the horizon) + per-iteration dynamic factors (`fallingEval` with deltas x2.08 into internal units, `timeReduction` 1.37/0.65 carried across moves, `bestMoveInstability`). The formula's steady state is ~0.5*optimum - which is why the 2.6.4 attempt (multiplying the fixed slice) regressed and this one did not. MoveOverhead default 100->30 (the formula reserves it x52, and at 100 it collapsed bullet endgames into instant moves). `AssumedMovesToGo` removed from the profile.

| Term | Ref value | Piece | Description |
|---------|---------|-------|-------------|
| `TrappedRook` | S(55,13) x (1 + !canCastle) | Rook | Rook with =3 move squares - a serious positional error |
| `RookOnClosedFile` | S(10,5) penalty | Rook | Rook on a file blocked by a friendly pawn |
| `BishopPawns` | -3 to -24 MG per pawn | Bishop | Friendly pawns on the bishop's color x distance to the edge |
...
- **Safety ladder of the path** - k = 36/30/17/7/0 (+5 if blockSq defended or own rook/queen behind); enemy rook/queen behind the past disputes the entire span. `(k*w, k*w)` in reference units, x0.48 per pawn at the end.
...
### [DONE] 4G - Additional Pawn Structure (v2.6.7) - DONE
...
Implemented (true to the reference, pawns.cpp) `evaluate()`, all x0.48). The key: the reference pawn scoring is a chain of MUTUALLY EXCLUSIVE branches (connected / isolated / backward), not a sum of independent terms - the entire chain was ported and the old additive terms (DoubledPawn per column, IsolatedPawn, Phalanx[], BackwardPawn texel-tuned) were removed.

| Term | Ref. Value | Status | Description |
|---------|---------|--------|-------------|
| Complete Connected Formula | `Connected[r]*(2+phalanx-opposed) + 22*support`, e.g. `v*(r-2)/4` | [DONE] DONE | In raw reference units, x0.48 at the end; replaces simple Phalanx[] |
| `WeakUnopposed` | S(15,18) -> S(7,9) | [DONE] DONE | Over Isolated/Backward with free column in front (backward only outside a/h) |
| `WeakLever` | S(2,57) -> S(1,27) | [DONE] DONE | Unsupported pawn attacked by two enemy pawns |
| `DoubledEarly` | S(17,7) -> S(8,3) | [DONE] DONE | Doubled while no enemy pawn is fixed |
| `BlockedPawn` rows 5-6 | {S(-19,-8), S(-7,3)} -> {(-9,-4),(-3,1)} | [DONE] DONE | Blocked advanced own pawn limits opponent |
| `Doubled` semantics ref | S(11,51) -> S(5,25) | [DONE] DONE | Own pawn RIGHT behind and unsupported (not the column count) |
| `Isolated` / `Backward` ref | S(1,20)/S(6,19) -> S(0,10)/S(3,9) | [DONE] DONE | The old texel values described other events (different branches) |

---

### [DONE] 4H - Material imbalance (v2.6.8) - DONE
...
- Combined factor x3/100 (reference /16 x x0.48 NoaChess). Pure White-Black difference: exactly zero for symmetric material.
...
### [DONE] 4H.1 - Timeman Patch: Bullet Sustainability Guardrail (v2.6.8)
...
### [DONE] 4I - Scaling Factors / Winnable (v2.6.9) - DONE
...
- **Complexity metric** - `9*passers + 12*pawns + 9*outflanking + 21*bothFlanks + 24*infiltration + 51*purePawnEndgame - 43*almostUnwinnable - 110`. In raw reference units and converted x0.48 once (mg/eg caps are NoaChess centipawns). Can only reduce mg; pushes eg in both directions; never changes the sign of either.
- **`almostUnwinnable`** - crossed kings (outflanking < 0) with all pawns on one flank -> -43 complexity.
- **Scale factors** (e.g., the mix is multiplied by sf/64, dimensionless ratios NOT x0.48-rescaled): material factor first - strong side without pawns and <= one bishop advantage: sf=0 under a rook (KK/KBK/KNK), 4 against a minor side only (KRKB/KRKN), 14 otherwise (KmmKm). General heuristics if not: pure OCB `18+4*strongPassers`; OCB with more material `22+3*strongUnits`; single rook endgame with <=1 pawn advantage, strong pawns on one flank and weak king defending -> 36; queen vs no queen `37+3*queenlessMinors`; rest of chapter `36+7*strongPawns` (-4 extra on a single flank); -4 final on any branch with all pawns on one flank (the default branch accumulates -8, verified character by character against the source of the reference).
...
### [DONE] 5A - Improving flag (v2.7.0) - DONE (+4.0 +/-27.1 SPRT STC; +43 +/-23 relative in gauntlet LTC, ~2965 +/-25 CCRL)
...
- **RFP (futility margin)** - `85 x (depth - improving)`: the reference form `165 x (depth - improving)`, already in its equivalent x0.48 (85/ply).
- **LMP** - threshold `3 + d^2` halfway through if not improving.
...
### [DONE] 5B - NMP and futility refined (v2.7.1) - RANGE TRIMMED BY MEASUREMENT - DONE
...
**History (2026-07-17):** The full reference bundle was faithfully implemented (input condition with margin improvement/complexity, statScore filter, deep R, verification, capture futility, quiet futility by lmrDepth - all x0.48 where appropriate and recalibrated x0.28 measured at history thresholds) and SPRT failed it: **[0.451], -34 Elo at 143 games**. Dissection with WAC-300 + node benches across seven builds identified THREE ecosystem dependencies:
...
3. **The futility by lmrDepth requires the large reference reductions** (its lmrDepth is systematically lower) - and **pruning margins do NOT carry x0.48**: the crudes reproduce our validated margins (d3: 251 vs 300; d4: 396 vs 400); the scaled ones double prune and blind. -> 5C.
...
- **StatScore Stack** - `2*butterfly + contHist - 1250` per ply (thresholds x0.28: ratio measured between our gravity-free depth^2 tables and the reference gravity-capped tables; probe: butterfly p99 3218, contHist p99 630 vs caps 14365/29952).
...
- **NMP verification at a depth >= 14** with `nmpMinPly = ply + 3(depth-R)/4` (anti-zugzwang, Fine 70 ok).
...
**Measured result:** WAC 258-265/300 vs 259 of 2.7.0 (equal within noise +/-5), nodes d15 2.92M vs 3.72M (-21%), startpos d16 2.25M vs 4.10M (-45%), mate WAC.001 to d13 (parity), Fine 70 ok, 138 tests.

---

### [CUT] 5C - LMR adjuster suite + statScore (era v2.7.2) - MEASURED AND CUT (2026-07-18)
...
2. **The term "history" in LMR had been dead for some time.** It was `Clamp(butterfly/16384, +/-2)`Calibrated for a table that reaches its rescale of 2^20. Actually measured: butterfly p99 2840, max 6086. That division yielded **0 in over 99% of the quiet plays**.
...
|--------|---------|---------------|
| cutNode | +2 (r += 4026) | [CUT] CUT at both magnitudes at the actual rate (-4.0 H0 at 4026; -7.1 H0 at 1536); the yarn remains (neutral, used by the following) |
| PvNode / ttPv | -1 - 11/(3+depth) / -2 | [DONE] **EMBARKED v2.8.4** as ttPv x0.34 (+7.5 screen) |
| ttCapture in play TT | +1 (r += 1079) | [DONE] **EMBARCADO v2.8.4** raw (+7.1 screen) |

**statScore** - [WARNING] **THE FORMULA BELOW WAS OUTDATED. Corrected against the source on disk on 2026-07-23.**
...
### [DONE] 5D - Improvements in TT (v2.7.2) - DONE (was 5F, renumbered to the actual execution order after the 5C cut)
...
- **Clustering** [DONE] - exact 16-byte entry (key32 + score int32 + eval int32 + move16 + depth8 + genBound8) -> **4 entries per 64B cache line** (the reference puts 3*10B into 32B with int16 scores; our +/-100k mates maintain int32 and the 4-byte cluster compensates without rescaling the mate scale).
- **Aging / Generation** [DONE] - 5-bit generation (cycle 32) at the beginning of each "go"; replacement by `depth - 8*edad_relativa` (exact formula of the reference); a hit refreshes the generation.
- **Static evaluation in TT** [DONE] - Hit serves the cached evaluation without an evaluator; miss saves evaluation-only input (bound None: never cuts off, never evicts real results). **+24% nps measured.**
- **PV flag in TT** [DONE] stored and sticky - **NO consumer in LMR on purpose**: the ttPv -2 was measured at 5C by exploding the PV subtree; with the actual flag already stored, that adjuster will be tested PER GAME in a later block.
- Reference overwrite rule (Exact fresh always; bound >4 plies more superficial never; best move and PV mark survive).
- Benchmark: -19% nodes, +24% nps, WAC 265/300 (record), Fine 70 ok, KRK ok, 184 tests (7 new TT tests).

---

### [CUT] 5E - Double extensions + earlier singular - MEASURED AND CUT (2026-07-19, campaign v2.7.3)
...
**The "not before NNUE" condition was honoured, and the verdict held (2026-08-27):** rebuilt
faithfully in the NNUE era - multi-cut, negative extensions, double/triple tiers, true margins -
and H0 again: -29.0 alone, -27.6 without multi-cut, -22.6 as the search package. **And the root
cause's own prescription was then tested end to end (2026-08-30):** the brake it demanded
(r += 4026 + 933 cutNode) was measured on the modern pipeline - IIR present, statScore ON - and
lost ~17 Elo (H0). Neither the accelerator nor the brake transfers to this engine's milder
reduction curve. Closed on current evidence at every link of the chain.

---

### [DONE] 5F - ProbCut rework + capture history - RE-AUDITED AND ONBOARDED IN v2.8.2
...
- **Margin sensitive to improving**: `beta + 150 - 40*improving`The base remains OUR game-validated 150 - the reference 241 measured worse here in nodes because its margin presupposes its eval/qsearch accuracy (same scaling lesson as 4B/4C/5B). The subtrahend is its 64 rescaled to our base (64 x 150/241 ~ 40).
...
### [DONE] 5G - History and Sorting - CAPTURE HISTORY IN v2.8.1; GRAVITY IN v2.8.2, CONTINUOUS BANDS CUT
...
2. **The blend should not reach the statScore** - the RFP thresholds (offset 1250, divisor 180, transfer x0.28) describe a one-level signal.
...
### [DONE] 5H - Aspiration, draw detection, check extension - RE-AUDITED AND TRIMMED IN v2.8.2
...
## [DONE] BLOCK 9 - Syzygy Endgame Tables (v2.8.0 - ADVANCED, before the NNUE)
...
## [DONE] BLOCK 6 - NNUE - Production (v3.0.0)
...
### [DONE] 6A - Feature encoding + incremental update successful - DONE (v3.0.0)
...
- **`NnueFeatureIndex.cs`** - implement **HalfKP** first (simpler than the reference HalfKAv2-hm):
  - Index = `king_square x 640 + piece_type_color x 64 + piece_square`
  - 41,024 possible inputs (64 king positions x 640 piece features)
...
### [DONE] 6B - Data generation from self-play - DONE (v3.0.0, with deviations)
...
### [DONE] 6D - NNUE activation in production - DONE (v3.0.0, kernel)
...
## [DONE] BLOCK 7 - NNUE iterative self-play (v3.1.0)
...
- [WARNING] Playing against a strong engine DOES NOT work: the games are unbalanced (positions with huge advantage) and the tags are still bad because the tag search is on a board that has already been decided.
...
|---|---|---|---|
| Feature transformer width | **128** | 1024 | **8x narrower** |
| Output buckets | **1** | 8 (by piece count) | - |
| Positions per generation | **13.1 M** | billions across training | **~100x fewer** |
| Samples seen per run | **~550 M** (7 gens x 6 epochs) | tens of billions | **~50x fewer** |
| Net file size | **5.7 MB** | 45-70 MB | - |
| L1 arithmetic | int16 (`VPMADDWD`) | int8 (`VPDPBUSD`/emulated) | **2-4x slower** |
...
product is 32 x 256 = **8,192 int16 MACs ~ 512 AVX2 instructions per evaluation** - far too small
...
### [DONE] v4.0.0 - Foundation - DONE (2026-07-31, no Elo claim, shipped net unchanged)
...
MoveFeature 387.2 -> **154.9 ns** (2.5x), Add/Subtract 270.9 -> **145.0 ns**, CopyFrom 138.2 -> **62.2
...
**Accumulator cache (finny table).** Refresh **4407 -> 110 ns (40-52x cheaper)**, 99.6% served from
...
rather than a speedup today - at ft=1024 the same rebuild is 8x more expensive.
...
records/s, 12x faster, 300M in 29 minutes.** The scalar `record_to_features` stays as the definition
...
#### [DONE] PHASE 0 MEASURED (2026-08-01) - the premise held, and by a landslide

Two arms matched on **total search work**, not position count (at 28k nodes a position costs ~4.7x
...
22,528 x 128 ~ 2.9M parameters, so the deep arm had ~1.5 positions per parameter and was broken from
...
- **Drop labelling depth to 5,000-8,000 nodes.** At this network size quantity beats label quality by
  ...
  roughly 5x more positions per hour of machine time.
...
tests, Python run against the same table, identical everywhere and in range over piece counts 0-40 x
...
Cost rises **sub-linearly**: doubling the transformer costs ~1.5x, not 2x, because packing and the
...
- **Delete the classical evaluator** (~2,300 lines across `ClassicalEvaluator`, `EvaluationParams`,
  ...
  tuning a superseded path (see section FINAL REVIEW, which still holds King Safety Phase B and
...
still reached, by a different route: `fqcohuman3` (v5.9.2; embedded until v5.9.10, when `fqco592`
replaced it) gauntlets at **3321 +/- 45 CCRL**.
...
- Calculate for each position and move: `peso = frecuencia x (win_rate - 0.5 x draw_rate)`.
...
- **Search survives the NNUE; evaluation does not.** These two are terms WITHIN the classic evaluator, and the integration plan (section 6, blending) leaves the classic as a marginal fallback - the reference only consults it with more than 7 pieces AND `|psq| > 1760`Anything refined here will no longer be consulted.
...
| (5C) | [CUT] CUT - suite LMR + statScore 4-comp: all negative to 10+0.1 (-9.7/-25.7/-11.5/-10.8); fix contHist ply2/4 archived -> 5G | - | - |
| 2.7.2 | 5D TT redesign (5F era): 4*16B clustering, aging, cached eval, ttPv | +37.9 +/-15.0 grouped; +48 +/-23 rel LTC | **~2975 +/-25 measured** |
| (2.7.3) | [CUT] CUT WITHOUT RELEASE - 5E single campaign (4 negative SPRTs) + 5G multi-level story (4 builds, the last 2 exact equity; below tables-by-distance/gravity/gate built, blocking = hard gang killers/counter); both closed | - | - |
...
| 2.8.2/5F | [DONE] ProbCut reimplemented with normal depth>=1 verification; A/B isolated 59-51-90 | +13.9 +/-35.8, LOS 77.7% | Included in full match |
...
| **3.1.0** | **Lazy SMP (16 threads) - [DONE] DONE** - `Threads=1` byte-identical, node scaling ~7.6x to 8 threads | LTC gauntlet pending | ~3150+ expected |
| **3.1.1** | **Cold-start patch - [DONE] DONE** - `PublishReadyToRun` AOT: start ~25 s -> ~7 s; warmup NNUE depth 6 -> 1. No change in strength | - | = **~3050 CCRL** |
| **3.1.2** | **Time Fix - [DONE] DONE** - cap at mid-iteration to 1 thread + easy-move to \|score\|>=700cp; 8.5s->1.68s in decisive final to 5+5 | SPRT -5.0 +/-27.7 (neutral); published by direct measurement | = **~3050 CCRL** |
| **3.2.0** | **NNUE gen7 + human openings (block 8) - [DONE] DONE** - `pgnbook` Extracts 3.04M elite FENs and seeds the datagen | vs gen5 +3.7 (parity); vs classic +28.5 LOS 100% | **~3080 +/-40 CCRL** |
| **3.2.1** | **Bot Stability - [DONE] DONE** - depth stop in ponder + STALL visible; `Threads` 30->24 in the bot config | no strength change | = ~3080 |
